...
async function evaluatePolicies(agentId: string, action: string, context: any): Promise<{
  approved: boolean;
  tier: string;
...
// Warrant issuance
async function issueWarrant(executionId: string, evaluation: any): Promise<string> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { action, agent_id, context } = req.body as any;
