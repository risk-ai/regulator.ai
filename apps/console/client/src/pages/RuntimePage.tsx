    ...
    <PageLayout title="Runtime" description="Execution pipeline health and metrics">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: stats?.health === 'healthy' ? '#4ade80' : '#f87171' }} />
          <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', textTransform: 'uppercase' }}>
            {stats?.health || 'loading'}
          </span>
        </div>
        {connected && (
          <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '4px', background: 'rgba(74, 222, 128, 0.08)', color: '#4ade80', border: '1px solid rgba(74, 222, 128, 0.2)', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>SSE LIVE</span>
        )}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '4px' }}>
          {['5m', '1h', '24h', '7d'].map(w => (
            <button key={w} onClick={() => setWindow(w)} style={{
              ...
              color: window === w ? '#a78bfa' : 'var(--text-tertiary)', cursor: 'pointer', fontFamily: 'var(--font-mono)',
            }}>{w}</button>
          ))}
        </div>
      </div>
            ...
            <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-subtle)', borderRadius: '10px', overflow: 'hidden' }}>
              {executions.length === 0 ? (
                ...
                <div key={ex.execution_id} style={{
                  ...
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: statusColors[ex.status] || '#94a3b8', flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{ex.action}</span>
                      <span style={{ fontSize: '10px', fontWeight: 600, color: statusColors[ex.status] || '#94a3b8', fontFamily: 'var(--font-mono)', padding: '1px 5px', borderRadius: '3px', background: `${statusColors[ex.status] || '#94a3b8'}12` }}>{ex.risk_tier}</span>
                      <span style={{ fontSize: '10px', padding: '1px 5px', borderRadius: '3px', background: `${statusColors[ex.status] || '#94a3b8'}08`, color: statusColors[ex.status] || '#94a3b8', fontFamily: 'var(--font-mono)' }}>{ex.status}</span>
                    </div>
                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '1px' }}>{ex.agent_name || 'system'} · {ex.approved_by || 'auto'} · {ex.executed_at ? new Date(ex.executed_at).toLocaleString() : ''}</div>
                  </div>
                </div>
              ))}
            </div>
...
function MetricCard({ label, value, suffix, color }: { label: string; value: number; suffix?: string; color: string }) {
  return (
    <div style={{ background: `${color}08`, border: `1px solid ${color}15`, borderRadius: '10px', padding: '14px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '2px' }}>
        <span style={{ fontSize: '24px', fontWeight: 700, color, fontFamily: 'var(--font-mono)', lineHeight: 1 }}>{value}</span>
