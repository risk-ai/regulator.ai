...
export async function storeRefreshToken(userId, token) {
    ...
    const result = await queryOne(`INSERT INTO refresh_tokens (user_id, token_hash, expires_at)
     VALUES ($1, $2, $3)
...
export async function validateRefreshToken(token) {
    try {
        ...
        const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
        const dbToken = await queryOne(`SELECT user_id, revoked_at FROM refresh_tokens 
       WHERE token_hash = $1 AND expires_at > NOW()`, [tokenHash]);
        ...
        const user = await queryOne(`SELECT id, tenant_id, email, role FROM users WHERE id = $1`, [dbToken.user_id]);
        if (!user) {
            return null;
        }
        return {
...
export async function cleanupExpiredTokens() {
    const result = await queryOne(`DELETE FROM refresh_tokens 
