...
export function createActionsRouter() {
     ...
     * }
     */
    router.post('/execute', jwtAuthMiddleware, async (req, res) => {
        ...
        try {
            ...
            const actionTypeRecord = await queryOne('SELECT id, action_type, display_name, enabled, requires_approval FROM action_types WHERE action_type = $1', [action_type]);
            if (!actionTypeRecord) {
            ...
            // Log usage
            const usageId = await execute(`INSERT INTO action_type_usage (action_type_id, agent_id, status)
         VALUES ($1, $2, 'submitted')
         RETURNING id`, [actionTypeRecord.id, req.user?.userId || 'unknown']);
            ...
            // Update usage log
            await execute(`UPDATE action_type_usage
         SET status = $1, execution_time_ms = $2
         WHERE id = $3`, [result.success ? 'executed' : 'failed', executionTime, usageId]);
            if (result.success) {
                return res.json({
                    success: true,
                    data: result.data,
     ...
     */
    router.get('/types', jwtAuthMiddleware, async (req, res) => {
        try {
            // Check authentication
            if (!req.user) {
                return res.status(401).json({
      ...
      `);
            // Group by category
            const byCategory = actionTypes.reduce((acc, type) => {
