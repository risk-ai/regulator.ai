...
export function createActivityFeedRouter(viennaRuntime) {
    const router = Router();
    /**
     * Get activity feed
    ...
    router.get('/feed', async (req, res) => {
        try {
            ...
            const { limit = 50, offset = 0, agent_id, action_type, status, since, // ISO timestamp
    ...
    router.get('/summary', async (req, res) => {
        ...
        LIMIT 10000`, [tenantId, since.toISOString()]);
            ...
            summary.top_agents = Object.entries(summary.actions_by_agent)
                .map(([agent_id, count]) => ({ agent_id, count: count }))
                .sort((a, b) => b.count - a.count)
                .slice(0, 10);
            // Top actions
            summary.top_actions = Object.entries(summary.actions_by_type)
                .map(([action_type, count]) => ({ action_type, count: count }))
                .sort((a, b) => b.count - a.count)
                .slice(0, 10);
            res.json({
                success: true,
    ...
    router.get('/timeline', async (req, res) => {
        try {
            const authReq = req;
            const tenantId = getTenantId(authReq);
            const { period = '24h', interval = '1h' // 5m, 15m, 1h, 1d
             } = req.query;
            // Calculate time range
            let since = new Date();
            switch (period) {
        ...
        LIMIT 10000`, [tenantId, since.toISOString()]);
            ...
            events.forEach((event) => {
                ...
                const intervalStart = new Date(Math.floor(eventTime.getTime() / intervalMs) * intervalMs);
            ...
            const timelineArray = Object.values(timeline).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
            res.json({
                success: true,
                data: timelineArray,
                period,
    ...
    router.get('/stream', (req, res) => {
        ...
        res.flushHeaders();
        // Subscribe to Vienna runtime events
        ...
        // Add listener
        viennaRuntime.on('execution_event', eventHandler);
        // Cleanup on disconnect
        req.on('close', () => {
            viennaRuntime.off('execution_event', eventHandler);
        });
