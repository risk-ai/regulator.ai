...
export function createAgentTemplatesRouter() {
    const router = Router();
    /**
    ...
    router.get('/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const template = await queryOne('SELECT * FROM agent_templates WHERE id = $1 AND enabled = true', [id]);
            if (!template) {
    ...
    router.get('/framework/:framework', async (req, res) => {
        try {
            ...
            const templates = await query(`SELECT * FROM agent_templates
         WHERE framework = $1 AND enabled = true
         ORDER BY use_count DESC, name ASC`, [framework]);
            res.json({
                success: true,
                data: templates,
                framework,
    ...
    router.get('/stats/popular', async (req, res) => {
        try {
            ...
            const templates = await query(`SELECT id, name, description, framework, icon, use_count, tags
