...
export function createAnalyticsRouter() {
    ...
    router.get('/agents', async (req, res) => {
        ...
        WHERE a.tenant_id = $2
      `;
            const params = [since.toISOString(), tenantId];
            if (agent_id) {
    ...
    router.get('/organization', async (req, res) => {
        ...
        WHERE tenant_id = $1 AND started_at >= $2`, [tenantId, since.toISOString()]);
            ...
            // Get top agents
            const topAgents = await query(`SELECT 
          a.agent_id,
          a.display_name,
        ...
        LIMIT 10`, [tenantId, since.toISOString()]);
            // Get policy violations (approximation)
            const policyViolations = await query(`SELECT COUNT(*) as count
        FROM approval_requests
        WHERE tenant_id = $1 AND requested_at >= $2`, [tenantId, since.toISOString()]);
            res.json({
                success: true,
                data: {
                    period,
    ...
    router.get('/policies', async (req, res) => {
        try {
            ...
            const policyStats = await query(`SELECT 
          p.id,
          p.name,
        ...
        ORDER BY trigger_count DESC`, [since.toISOString(), tenantId]);
            res.json({
                success: true,
                data: {
                    period,
