...
import { generateAccessToken, generateRefreshToken, storeRefreshToken, validateRefreshToken, revokeRefreshToken, jwtAuthMiddleware, } from '../middleware/jwtAuth.js';
import { createApiKey, revokeApiKey, } from '../middleware/apiKeyAuth.js';
const router = Router();
...
router.post('/register', async (req, res) => {
    try {
        const { email, password, name, company, plan = 'community' } = req.body;
        if (!email || !password) {
        ...
        const result = await transaction(async () => {
            ...
            const tenant = await queryOne(`INSERT INTO tenants (name, slug, plan, max_agents, max_policies, settings)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`, [
            ...
            // Create user
            const user = await queryOne(`INSERT INTO users (tenant_id, email, password_hash, name, role)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id`, [tenant.id, email.toLowerCase(), passwordHash, name || null, 'admin']);
            if (!user) {
...
router.post('/login', async (req, res) => {
    try {
        ...
        // Find user
        const user = await queryOne(`SELECT u.id, u.tenant_id, u.email, u.password_hash, u.name, u.role
       FROM users u
...
router.get('/me', jwtAuthMiddleware, async (req, res) => {
    try {
        ...
        // Get fresh user data
        const user = await queryOne(`SELECT id, email, name, role, last_login_at, created_at 
       FROM users 
       WHERE id = $1`, [req.user.userId]);
        ...
        // Get tenant data
        const tenant = await queryOne(`SELECT id, name, slug, plan, max_agents, max_policies 
       FROM tenants 
       WHERE id = $1`, [req.user.tenantId]);
        res.json({
            success: true,
            data: {
                user,
 ...
 */
router.post('/api-keys', jwtAuthMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({
                success: false,
 ...
 */
router.get('/api-keys', jwtAuthMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({
                success: false,
        ...
        const apiKeys = await query(`SELECT id, name, key_prefix, scopes, agent_id, rate_limit, 
              last_used_at, expires_at, revoked_at, created_at
       FROM api_keys 
       WHERE tenant_id = $1 AND revoked_at IS NULL
       ORDER BY created_at DESC`, [req.user.tenantId]);
        res.json({
            success: true,
            data: {
 ...
 * Revoke API key
 */
router.delete('/api-keys/:id', jwtAuthMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({
                success: false,
        ...
        const apiKey = await queryOne('SELECT id FROM api_keys WHERE id = $1 AND tenant_id = $2', [id, req.user.tenantId]);
        if (!apiKey) {
            return res.status(404).json({
                success: false,
