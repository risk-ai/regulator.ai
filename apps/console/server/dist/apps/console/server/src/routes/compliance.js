...
export function createComplianceRouter() {
    ...
    router.get('/reports', async (req, res) => {
        try {
            ...
            res.status(500).json({
                success: false, error: error.message, code: 'LIST_REPORTS_ERROR',
                timestamp: new Date().toISOString(),
            });
        }
    });
    router.get('/reports/:id', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'GET_REPORT_ERROR', timestamp: new Date().toISOString() });
        }
    });
    router.post('/reports/generate', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'GENERATE_ERROR', timestamp: new Date().toISOString() });
        }
    });
    router.delete('/reports/:id', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'DELETE_ERROR', timestamp: new Date().toISOString() });
        }
    });
    ...
    router.get('/reports/:id/pdf', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'PDF_ERROR', timestamp: new Date().toISOString() });
        }
    });
    // ─── CSV Export ─────────────────────────────────────────────────────────
    router.get('/reports/:id/csv', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'CSV_ERROR', timestamp: new Date().toISOString() });
        }
    });
    // ─── Templates ──────────────────────────────────────────────────────────
    router.get('/templates', async (_req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'TEMPLATES_ERROR', timestamp: new Date().toISOString() });
        }
    });
    router.post('/templates', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'CREATE_TEMPLATE_ERROR', timestamp: new Date().toISOString() });
        }
    });
    router.put('/templates/:id', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'UPDATE_TEMPLATE_ERROR', timestamp: new Date().toISOString() });
        }
    });
    // ─── Scheduling ─────────────────────────────────────────────────────────
    router.post('/schedule', async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, error: error.message, code: 'SCHEDULE_ERROR', timestamp: new Date().toISOString() });
        }
    });
    return router;
}
...
function generatePdfHtml(report) {
    ...
    if (sections.executive_summary) {
      ...
      <div class="section">
        ...
        ${es.highlights ? `<div class="highlights">${es.highlights.map((h) => `<p class="highlight-item">• ${h}</p>`).join('')}</div>` : ''}
      </div>`;
    }
    // Governance Overview
    if (sections.governance_overview) {
      ...
      <div class="section">
        ...
        ${go.action_distribution?.length ? `
          ...
          <div class="bar-chart">
            ${go.action_distribution.map((d) => {
            const max = Math.max(...go.action_distribution.map((x) => x.count));
    ...
    if (sections.action_volume) {
      ...
      <div class="section page-break">
        ...
        ${av.by_risk_tier?.length ? `
          ...
          <div class="bar-chart">
            ${av.by_risk_tier.map((t) => {
            const max = Math.max(...av.by_risk_tier.map((x) => x.count));
        ...
        ${av.daily_trend?.length ? `
          ...
          <div class="mini-chart">
            ${av.daily_trend.map((d) => {
            const max = Math.max(...av.daily_trend.map((x) => x.count));
      ...
      <div class="section">
        ...
        ${pc.top_triggered_rules?.length ? `
          ...
          <table class="data-table">
            ...
            <tbody>${pc.top_triggered_rules.map((r) => `
      ...
      <div class="section">
        ...
        ${ra.top_denials?.length ? `
          ...
          <table class="data-table">
            ...
            <tbody>${ra.top_denials.map((d) => `<tr><td>${d.agent_id}</td><td>${d.action_type}</td><td>${d.count}</td></tr>`).join('')}</tbody>
          </table>` : ''}
      </div>`;
    }
    // Approval Metrics
    if (sections.approval_metrics) {
      ...
      <div class="section">
        ...
        ${am.by_tier?.length ? `
          ...
          <div class="bar-chart">
            ${am.by_tier.map((t) => {
            const max = Math.max(...am.by_tier.map((x) => x.count));
            const pct = max > 0 ? (t.count / max) * 100 : 0;
      ...
      <div class="section page-break">
        ...
        ${vi.by_severity?.length ? `
          ...
          <div class="severity-pills">
            ${vi.by_severity.map((s) => `<span class="badge ${s.severity === 'critical' ? 'badge-red' : s.severity === 'warning' ? 'badge-yellow' : 'badge-blue'}">${s.severity}: ${s.count}</span>`).join(' ')}
          </div>` : ''}
        ${vi.incident_timeline?.length ? `
          ...
          <table class="data-table">
            ...
            <tbody>${vi.incident_timeline.slice(0, 20).map((i) => `
              <tr>
    ...
    if (sections.recommendations) {
      ...
      <div class="section">
        ...
        ${rec.items?.length ? rec.items.map((r) => `
