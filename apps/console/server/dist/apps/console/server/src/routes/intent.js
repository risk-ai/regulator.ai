...
export function createIntentRouter() {
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const tenant_id = req.session?.tenant_id || 'system';
            ...
            let defaultRiskTier;
            try {
                actionType = await queryOne('SELECT id, action_type, display_name, default_risk_tier, enabled FROM action_types WHERE action_type = $1', [intent_type]);
            }
            catch (dbError) {
            ...
            if (actionType) {
                ...
                try {
                    await execute(`INSERT INTO action_type_usage (action_type_id, agent_id, intent_id, status)
             VALUES ($1, $2, $3, 'submitted')`, [actionType.id, operator_id, null]);
                }
                catch (usageError) {
            ...
            if (actionType) {
             ...
             LIMIT 1`, [status, response.intent_id || null, actionType.id, operator_id]);
                }
                catch (updateError) {
