...
export function createPolicyTemplatesRouter() {
    const router = Router();
    /**
    ...
    router.get('/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const template = await queryOne('SELECT * FROM policy_templates WHERE id = $1 AND enabled = true', [id]);
            if (!template) {
    ...
    router.post('/:id/instantiate', async (req, res) => {
        try {
            ...
            // Create policy from template
            const policy = await queryOne(`INSERT INTO policies (
          tenant_id,
          name,
          description,
          enabled,
        ...
        RETURNING *`, [
            ...
            await query('SELECT increment_template_use_count($1)', [id]);
            res.json({
                success: true,
                data: policy,
    ...
    router.get('/stats/popular', async (req, res) => {
        try {
            ...
            const templates = await query(`SELECT id, name, description, category, icon, use_count
    ...
    router.get('/categories/:category', async (req, res) => {
        try {
            ...
            const templates = await query(`SELECT * FROM policy_templates
         WHERE category = $1 AND enabled = true
         ORDER BY use_count DESC, name ASC`, [category]);
            res.json({
                success: true,
                data: templates,
                category,
