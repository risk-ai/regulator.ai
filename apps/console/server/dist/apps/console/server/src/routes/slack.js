...
export function createSlackRouter() {
    ...
    router.get('/status', async (req, res) => {
        try {
            ...
            const workspace = await queryOne('SELECT id, team_id, team_name, channel_approvals, channel_alerts, enabled, installed_at FROM slack_workspaces WHERE tenant_id = $1', [tenantId]).catch(() => null);
            if (!workspace) {
                return res.json({
                    success: true,
                    data: {
    ...
    router.get('/oauth/callback', async (req, res) => {
        try {
            ...
            // Install workspace
            const workspace = await slackService.installWorkspace(tenantId, code, userId);
    ...
    router.post('/configure', async (req, res) => {
        try {
            ...
            await query(`UPDATE slack_workspaces
         SET channel_approvals = $1,
             channel_alerts = $2
         WHERE tenant_id = $3`, [channel_approvals, channel_alerts, tenantId]);
            res.json({
                success: true,
    ...
    router.post('/toggle', async (req, res) => {
        try {
            ...
            await query('UPDATE slack_workspaces SET enabled = $1 WHERE tenant_id = $2', [enabled, tenantId]);
            res.json({
                success: true,
