...
export class BillingService extends EventEmitter {
    ...
    async recordUsage(event) {
        try {
            ...
            await query(`INSERT INTO usage_events (tenant_id, event_type, count, period, metadata, recorded_at)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (tenant_id, event_type, period) 
         DO UPDATE SET 
           count = usage_events.count + EXCLUDED.count,
           metadata = EXCLUDED.metadata,
           recorded_at = EXCLUDED.recorded_at`, [event.tenant_id, event.event_type, event.count, period, JSON.stringify(event.metadata || {}), now]);
    ...
    async getUsageMetrics(tenantId, period) {
        try {
            ...
            const events = await query(`SELECT event_type, SUM(count) as count
         FROM usage_events 
         WHERE tenant_id = $1 AND period = $2
         GROUP BY event_type`, [tenantId, currentPeriod]);
            // Get active agent count (distinct agents that sent heartbeat in last 24h)
            const activeAgents = await queryOne(`SELECT COUNT(DISTINCT agent_id) as count
         FROM usage_events 
         WHERE tenant_id = $1 
           AND event_type IN ('agent_registered', 'agent_heartbeat')
           AND recorded_at > NOW() - INTERVAL '24 hours'`, [tenantId]);
            // Get storage usage (latest storage event)
            const storageUsage = await queryOne(`SELECT count 
         FROM usage_events 
     ...
     */
    async checkPlanLimits(tenantId, action) {
        try {
     ...
     */
    async checkUsageLimits(tenantId, period) {
        try {
            const { limits } = await this.getTenantPlan(tenantId);
     ...
     */
    async recordBillingAlert(tenantId, alert) {
        try {
            const now = new Date().toISOString();
            // Check if we already sent this alert recently (within last 24h)
            const existingAlert = await queryOne(`SELECT id FROM billing_alerts 
         WHERE tenant_id = $1 
           AND alert_type = $2 
           AND metric = $3 
           AND threshold_percent = $4
           AND triggered_at > NOW() - INTERVAL '24 hours'`, [tenantId, alert.type, alert.metric, alert.threshold_percent]);
            if (existingAlert) {
                // Don't spam alerts - only once per 24h
                return;
            }
            await query(`INSERT INTO billing_alerts (tenant_id, alert_type, metric, threshold_percent, message, triggered_at)
         VALUES ($1, $2, $3, $4, $5, $6)`, [tenantId, alert.type, alert.metric, alert.threshold_percent, alert.message, now]);
        }
        catch (error) {
     ...
     */
    async generateMonthlySummary(tenantId, period) {
        try {
...
Total: $${(limits.price_monthly + overageCharges).toFixed(2)}
      `.trim();
            ...
            await query(`INSERT INTO usage_summaries (tenant_id, period, metrics, overage_charges, summary, generated_at)
         VALUES ($1, $2, $3, $4, $5, $6)
     ...
     */
    async reportStripeUsage(tenantId, period) {
        try {
            if (!this.stripe) {
            ...
            const tenant = await queryOne('SELECT stripe_customer_id FROM tenants WHERE id = $1', [tenantId]);
            ...
            if (metrics.intents_submitted > 0) {
                usageRecords.push(this.stripe.subscriptionItems.createUsageRecord('si_intent_usage', {
            ...
            if (metrics.storage_gb > 0) {
                usageRecords.push(this.stripe.subscriptionItems.createUsageRecord('si_storage_usage', {
     ...
     */
    async getBillingAlerts(tenantId, daysBack = 7) {
        try {
            const alerts = await query(`SELECT id, alert_type, metric, threshold_percent, message, triggered_at
         FROM billing_alerts 
         WHERE tenant_id = $1 
     ...
     */
    async getUsageTrends(tenantId, months = 6) {
        try {
            const results = [];
            const now = new Date();
