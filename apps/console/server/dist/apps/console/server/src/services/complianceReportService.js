...
export class ComplianceReportService {
    ...
    async listReports(limit = 50, offset = 0) {
        const [reports, countResult] = await Promise.all([
            query(`SELECT * FROM compliance_reports ORDER BY generated_at DESC LIMIT $1 OFFSET $2`, [limit, offset]),
            queryOne(`SELECT COUNT(*) as count FROM compliance_reports`),
        ]);
        return { reports, total: parseInt(countResult?.count || '0', 10) };
    }
    async getReport(id) {
        return queryOne(`SELECT * FROM compliance_reports WHERE id = $1`, [id]);
    }
    async deleteReport(id) {
        const result = await query(`DELETE FROM compliance_reports WHERE id = $1 RETURNING id`, [id]);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async listTemplates() {
        return query(`SELECT * FROM report_templates ORDER BY created_at`);
    }
    async createTemplate(template) {
        const result = await queryOne(`INSERT INTO report_templates (name, description, sections, report_type, is_default)
    ...
    async updateTemplate(id, updates) {
        ...
        values.push(id);
        return queryOne(`UPDATE report_templates SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values);
    }
    async getTemplate(id) {
        return queryOne(`SELECT * FROM report_templates WHERE id = $1`, [id]);
    }
    // ═══════════════════════════════════════════════════════════════════════════
    // Quick Stats (real-time dashboard numbers)
    // ═══════════════════════════════════════════════════════════════════════════
    async getQuickStats(periodDays = 30) {
        const periodStart = new Date();
        periodStart.setDate(periodStart.getDate() - periodDays);
        const startStr = periodStart.toISOString();
        const [totalActions, violations, approvalTimes, unauthorized, agentScores,] = await Promise.all([
            // Total actions governed
            queryOne(`SELECT COUNT(*) as count FROM agent_activity WHERE created_at >= $1`, [startStr]),
            // Policy violations
            queryOne(`SELECT COUNT(*) as count FROM agent_alerts 
         WHERE alert_type = 'policy_violation' AND created_at >= $1`, [startStr]),
            ...
            queryOne(`SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (evaluated_at - evaluated_at)) * 1000), 0) as avg_ms 
         FROM policy_evaluations WHERE evaluated_at >= $1 AND action_taken = 'require_approval'`, [startStr]),
            ...
            queryOne(`SELECT COUNT(*) as count FROM agent_activity 
         ...
         AND agent_id IN (
           ...
           WHERE alert_type = 'policy_violation' AND created_at >= $1
            ...
            queryOne(`SELECT COALESCE(AVG(trust_score), 0) as avg_score FROM agent_registry WHERE status = 'active'`),
        ]);
    ...
    async generateReport(request) {
        ...
        const report = await queryOne(`INSERT INTO compliance_reports (report_type, title, period_start, period_end, report_data, status, generated_by)
       VALUES ($1, $2, $3, $4, $5, 'generating', $6) RETURNING *`, [
        ...
        this.buildReportData(report.id, periodStart, periodEnd, sections, request.report_type)
            ...
            execute(`UPDATE compliance_reports SET status = 'failed', report_data = $1 WHERE id = $2`, [JSON.stringify({ error: err.message }), report.id]).catch(() => { });
        });
        return report;
    }
    async buildReportData(reportId, periodStart, periodEnd, sections, reportType) {
        ...
        for (const section of sections) {
            try {
                ...
                sectionData[section] = { error: `Failed to generate: ${err.message}` };
            }
        }
        const reportData = {
            metadata: {
        ...
        await execute(`UPDATE compliance_reports SET status = 'ready', report_data = $1 WHERE id = $2`, [JSON.stringify(reportData), reportId]);
    }
    // ═══════════════════════════════════════════════════════════════════════════
    // Section Builders
    // ═══════════════════════════════════════════════════════════════════════════
    async buildExecutiveSummary(start, end) {
        const [totalActions, denials, violations, agentHealth, incidents] = await Promise.all([
            queryOne(`SELECT COUNT(*) as count FROM agent_activity WHERE created_at >= $1 AND created_at <= $2`, [start, end]),
            queryOne(`SELECT COUNT(*) as count FROM agent_activity WHERE result = 'denied' AND created_at >= $1 AND created_at <= $2`, [start, end]),
            queryOne(`SELECT COUNT(*) as count FROM agent_alerts WHERE alert_type = 'policy_violation' AND created_at >= $1 AND created_at <= $2`, [start, end]),
            queryOne(`SELECT COALESCE(AVG(trust_score), 0) as avg_score, COUNT(*) as active_count FROM agent_registry WHERE status = 'active'`),
            query(`SELECT severity, COUNT(*) as count FROM agent_alerts WHERE created_at >= $1 AND created_at <= $2 GROUP BY severity`, [start, end]),
        ]);
        const total = parseInt(totalActions?.count || '0', 10);
    ...
    async buildGovernanceOverview(start, end) {
        const [activeRules, ruleDistribution, recentRules] = await Promise.all([
            queryOne(`SELECT COUNT(*) as count FROM policy_rules WHERE enabled = true`),
            query(`SELECT action_on_match, COUNT(*) as count FROM policy_rules WHERE enabled = true GROUP BY action_on_match`),
            query(`SELECT name, created_at, action_on_match FROM policy_rules 
    ...
    async buildActionVolume(start, end) {
        const [total, byType, byTier, dailyTrend, peakHours] = await Promise.all([
            queryOne(`SELECT COUNT(*) as count FROM agent_activity WHERE created_at >= $1 AND created_at <= $2`, [start, end]),
            query(`SELECT action_type, COUNT(*) as count FROM agent_activity 
         WHERE created_at >= $1 AND created_at <= $2 GROUP BY action_type ORDER BY count DESC`, [start, end]),
            query(`SELECT COALESCE(risk_tier, 'unknown') as risk_tier, COUNT(*) as count FROM agent_activity 
         WHERE created_at >= $1 AND created_at <= $2 GROUP BY risk_tier ORDER BY count DESC`, [start, end]),
            query(`SELECT DATE(created_at) as day, COUNT(*) as count FROM agent_activity 
         WHERE created_at >= $1 AND created_at <= $2 GROUP BY DATE(created_at) ORDER BY day`, [start, end]),
            query(`SELECT EXTRACT(HOUR FROM created_at)::int as hour, COUNT(*) as count FROM agent_activity 
    ...
    async buildPolicyCompliance(start, end) {
        const [evalCount, topRules, matchRate] = await Promise.all([
            queryOne(`SELECT COUNT(*) as count FROM policy_evaluations WHERE evaluated_at >= $1 AND evaluated_at <= $2`, [start, end]),
            query(`SELECT pe.rule_id, COALESCE(pr.name, 'Deleted Rule') as name,
         ...
         ORDER BY times_triggered DESC LIMIT 15`, [start, end]),
            query(`SELECT result, COUNT(*) as count FROM policy_evaluations 
    ...
    async buildAgentPerformance(start, end) {
        const agents = await query(`SELECT ar.agent_id, ar.display_name, ar.trust_score, ar.status,
    ...
    async buildRiskAnalysis(start, end) {
        const [highRisk, denials, scopeCreep, anomalies] = await Promise.all([
            query(`SELECT result, COUNT(*) as count FROM agent_activity 
         WHERE risk_tier = 'T2' AND created_at >= $1 AND created_at <= $2 GROUP BY result`, [start, end]),
            query(`SELECT agent_id, action_type, COUNT(*) as count FROM agent_activity 
    ...
    async buildViolationsIncidents(start, end) {
        const [bySeverity, timeline, unresolved] = await Promise.all([
            query(`SELECT severity, COUNT(*) as count FROM agent_alerts 
         ...
         GROUP BY severity ORDER BY CASE severity WHEN 'critical' THEN 1 WHEN 'warning' THEN 2 ELSE 3 END`, [start, end]),
            query(`SELECT id, agent_id, alert_type, severity, message, created_at, resolved FROM agent_alerts 
         WHERE created_at >= $1 AND created_at <= $2 
         ORDER BY created_at DESC LIMIT 50`, [start, end]),
            queryOne(`SELECT COUNT(*) as count FROM agent_alerts WHERE resolved = false AND created_at >= $1 AND created_at <= $2`, [start, end]),
        ]);
        return {
    ...
    async buildIntegrationHealth(start, end) {
        const [integrations, deliveryStats] = await Promise.all([
            query(`SELECT id, type, name, enabled, consecutive_failures, last_success, last_failure FROM integrations ORDER BY name`),
            query(`SELECT integration_id, COUNT(*) as total, COUNT(*) FILTER (WHERE success = true) as successes
    ...
    async buildRecommendations(start, end, existingSections) {
        ...
        const staleRules = await query(`SELECT pr.name, pr.id FROM policy_rules pr
    ...
    async createSchedule(req) {
        ...
        const report = await queryOne(`INSERT INTO compliance_reports (report_type, title, period_start, period_end, report_data, status, schedule_cron, recipients)
       VALUES ($1, $2, $3, $4, '{}', 'scheduled', $5, $6) RETURNING *`, [req.report_type, title, periodStart.toISOString(), periodEnd.toISOString(), req.schedule_cron, JSON.stringify(req.recipients || [])]);
        return report;
    }
    async listSchedules() {
        return query(`SELECT * FROM compliance_reports WHERE schedule_cron IS NOT NULL ORDER BY generated_at DESC`);
    }
    // ═══════════════════════════════════════════════════════════════════════════
    // Helpers
    // ═══════════════════════════════════════════════════════════════════════════
    calculatePeriodStart(reportType, end) {
    ...
    generateTitle(reportType, start, end) {
    ...
    generateHighlights(total, complianceRate, violations, fleetScore) {
        const highlights = [];
