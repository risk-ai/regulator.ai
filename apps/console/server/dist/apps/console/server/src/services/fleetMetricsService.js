...
export class FleetMetricsService {
    ...
    async getFleetSummary() {
        ...
        const statusCounts = await query(`SELECT status, COUNT(*)::text as count FROM agent_registry GROUP BY status`);
        const statusMap = {};
        let totalAgents = 0;
    ...
    `);
        // Actions by result
        const resultCounts = await query(`SELECT result, COUNT(*)::text as count FROM agent_activity WHERE created_at >= CURRENT_DATE GROUP BY result`);
        const actionsByResult = {};
        for (const row of resultCounts) {
            actionsByResult[row.result] = parseInt(row.count);
        }
        // Unresolved alerts count
        const alertCount = await queryOne(`SELECT COUNT(*)::text as count FROM agent_alerts WHERE resolved = false`);
        // Violations count (policy_violation alerts)
        const violationsCount = await queryOne(`SELECT COUNT(*)::text as count FROM agent_alerts WHERE resolved = false AND alert_type = 'policy_violation'`);
        // Top agents by volume today
        const topAgents = await query(`SELECT a.agent_id, COALESCE(r.display_name, a.agent_id) as display_name, COUNT(*)::text as count
       ...
       LIMIT 5`);
        ...
        const attention = await query(`SELECT agent_id, display_name,
         CASE
       ...
       LIMIT 10`);
        ...
        const trend = await query(`SELECT date_trunc('hour', created_at)::text as hour, COUNT(*)::text as count
       FROM agent_activity
    ...
    `, [agentId]);
        ...
        // Actions by type
        const byType = await query(`SELECT action_type, COUNT(*)::text as count
       FROM agent_activity WHERE agent_id = $1
       GROUP BY action_type ORDER BY count DESC`, [agentId]);
        const actionsByType = {};
        for (const row of byType) {
            actionsByType[row.action_type] = parseInt(row.count);
        }
        // Actions by result
        const byResult = await query(`SELECT result, COUNT(*)::text as count
       FROM agent_activity WHERE agent_id = $1
       GROUP BY result`, [agentId]);
        const actionsByResult = {};
...
//# sourceMappingURL=fleetMetricsService.js.map