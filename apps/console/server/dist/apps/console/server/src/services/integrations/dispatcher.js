 ...
 */
async function findMatchingIntegrations(event) {
    const integrations = await query(`SELECT * FROM integrations WHERE enabled = true AND consecutive_failures < $1`, [CIRCUIT_BREAK_THRESHOLD]);
    return integrations.filter(integration => {
        // Check event type match
        const eventTypes = Array.isArray(integration.event_types)
            ? integration.event_types
            : JSON.parse(integration.event_types);
        ...
        if (filters.risk_tiers && event.data.risk_tier) {
            ...
            if (!allowed.includes(event.data.risk_tier))
                return false;
        }
        return true;
    });
 ...
 */
async function logEvent(integrationId, eventType, payload, success, responseStatus, responseBody, latencyMs, errorMessage) {
    await execute(`INSERT INTO integration_events (integration_id, event_type, payload, response_status, response_body, latency_ms, success, error_message)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [integrationId, eventType, JSON.stringify(payload), responseStatus || null, responseBody || null, latencyMs || null, success, errorMessage || null]);
}
/**
 * Update integration health status
 */
async function updateHealth(integrationId, success, error) {
    if (success) {
        await execute(`UPDATE integrations SET last_success = NOW(), consecutive_failures = 0, updated_at = NOW() WHERE id = $1`, [integrationId]);
    }
    else {
        await execute(`UPDATE integrations SET last_failure = NOW(), last_error = $2, consecutive_failures = consecutive_failures + 1, updated_at = NOW() WHERE id = $1`, [integrationId, error || 'Unknown error']);
    }
}
/**
 * Dispatch an event to a single integration
 */
async function dispatchToIntegration(integration, event) {
    ...
    try {
        ...
        await logEvent(integration.id, event.type, event.data, result.success, result.response?.status, typeof result.response?.body === 'string' ? result.response.body.slice(0, 2000) : JSON.stringify(result.response)?.slice(0, 2000), latencyMs, result.error);
 ...
 */
export async function resetCircuitBreaker(integrationId) {
    await execute(`UPDATE integrations SET consecutive_failures = 0, last_error = NULL, updated_at = NOW() WHERE id = $1`, [integrationId]);
}
/**
 * Get dispatch statistics for an integration
 */
export async function getIntegrationStats(integrationId) {
    const row = await queryOne(`SELECT 
     ...
     WHERE integration_id = $1 AND created_at > NOW() - INTERVAL '30 days'`, [integrationId]);
    return {
