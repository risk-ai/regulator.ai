...
function buildCommentBody(event) {
    ...
    lines.push('|-------|-------|');
    if (data.risk_tier)
        lines.push(`| Risk Tier | ${tierEmoji} **${data.risk_tier}** |`);
    if (data.agent_id)
        lines.push(`| Agent | \`${data.agent_id}\` |`);
    if (data.action_type)
        lines.push(`| Action | \`${data.action_type}\` |`);
    if (data.intent_id)
        lines.push(`| Intent | \`${data.intent_id.slice(0, 12)}\` |`);
    if (data.approval_id)
        lines.push(`| Approval | \`${data.approval_id.slice(0, 12)}\` |`);
...
export const githubAdapter = {
    ...
    async sendNotification(event, config) {
        ...
        if (prNumber) {
            ...
            const result = await githubApi(`/repos/${owner}/${repo}/issues/${prNumber}/comments`, token, { method: 'POST', body: JSON.stringify({ body }) });
        ...
        if (commitSha) {
            ...
            const result = await githubApi(`/repos/${owner}/${repo}/statuses/${commitSha}`, token, {
                method: 'POST',
                body: JSON.stringify({
                    state,
    ...
    async testConnection(config) {
        ...
        if (!result.ok) {
            ...
            if (result.status === 404)
                return { success: false, message: `Repository ${config.owner}/${config.repo} not found or no access` };
