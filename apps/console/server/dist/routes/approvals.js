...
export function createApprovalsRouter(vienna) {
    const router = Router();
    /**
     ...
     */
    router.post('/:approval_id/approve', async (req, res) => {
        try {
            const { ApprovalManager } = await import('../../../../lib/core/approval-manager.js');
            const { getStateGraph } = await import('../../../../lib/state/state-graph.js');
            const stateGraph = getStateGraph();
            await stateGraph.initialize();
            const { approval_id } = req.params;
            ...
            const approval = await manager.approve(approval_id, reviewed_by, decision_reason || null);
            res.json({
                success: true,
                data: approval,
                timestamp: new Date().toISOString(),
     ...
     */
    router.post('/:approval_id/deny', async (req, res) => {
        try {
            const { ApprovalManager } = await import('../../../../lib/core/approval-manager.js');
            const { getStateGraph } = await import('../../../../lib/state/state-graph.js');
            const stateGraph = getStateGraph();
            ...
            const approval = await manager.deny(approval_id, reviewed_by, decision_reason);
            res.json({
                success: true,
                data: approval,
                timestamp: new Date().toISOString(),
