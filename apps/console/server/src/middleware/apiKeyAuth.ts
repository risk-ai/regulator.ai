 ...
 */
export function apiKeyAuthMiddleware(req: AuthenticatedApiRequest, res: Response, next: NextFunction) {
...
async function validateApiKey(apiKey: string): Promise<ApiKeyRecord | null> {
  ...
  const record = await queryOne<ApiKeyRecord>(
    `SELECT id, tenant_id, key_hash, key_prefix, name, scopes, agent_id, 
 ...
 */
async function checkRateLimit(keyId: string, limit: number): Promise<{ allowed: boolean; retryAfter?: number }> {
  const now = Date.now();
...
export async function createApiKey(params: {
  ...
  const result = await queryOne<{ id: string }>(
    `INSERT INTO api_keys (tenant_id, key_hash, key_prefix, name, scopes, agent_id, rate_limit, expires_at, created_by)
