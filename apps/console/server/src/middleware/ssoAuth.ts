...
export interface SSOConfig {
  ...
  // SAML fields
  saml_entry_point?: string;    // IdP SSO URL
  saml_issuer?: string;         // SP Entity ID
  saml_cert?: string;           // IdP public certificate
  saml_callback_url?: string;   // ACS URL
  
  // OIDC fields
  oidc_issuer?: string;         // e.g., https://accounts.google.com
  ...
  // Common
  default_role?: string;        // Role for JIT-provisioned users
  allowed_domains?: string[];   // Email domain allowlist
  force_sso?: boolean;          // Disable password login when SSO enabled
}
...
class SSOStateManager {
  private states: Map<string, { tenantId: string; createdAt: number }> = new Map();
  private readonly ttlMs = 600_000; // 10 minutes

  generate(tenantId: string): string {
  ...
  validate(state: string): { valid: boolean; tenantId?: string } {
 ...
 */
export function getOIDCAuthUrl(config: SSOConfig): string {
...
export async function jitProvision(
  ...
  const result = await db.queryOne(
    `INSERT INTO users (tenant_id, email, name, role, last_login_at)
     VALUES ($1, $2, $3, $4, NOW())
     RETURNING id`,
    [tenantId, email, name, defaultRole]
  );
 ...
 */
export function createSSORoutes(db: any, issueJWT: (user: any) => string) {
  const express = require('express');
  const router = express.Router();

  // Initiate SSO
  router.get('/sso/:tenantSlug', async (req: Request, res: Response) => {
    try {
      const { tenantSlug } = req.params;
      
      // Look up tenant SSO config
      const tenant = await db.queryOne(
        'SELECT id, settings FROM tenants WHERE slug = $1',
        [tenantSlug]
      );
      ...
      const ssoConfig = tenant.settings?.sso as SSOConfig | undefined;
  ...
  router.get('/sso/callback', async (req: Request, res: Response) => {
    try {
      ...
      const stateCheck = stateManager.validate(state as string);
      if (!stateCheck.valid) {
      ...
      const ssoConfig = tenant.settings?.sso as SSOConfig;
      ...
      // JIT provision user
      const { userId, created } = await jitProvision(
        db,
        tenant.id,
        tokenResult.email,
        tokenResult.name,
