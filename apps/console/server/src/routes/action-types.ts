...
interface CreateActionTypeBody {
  action_type: string;
  display_name: string;
  description?: string;
  category?: string;
...
interface UpdateActionTypeBody {
  display_name?: string;
  description?: string;
  category?: string;
  payload_schema?: Record<string, unknown>;
...
function validateActionTypeSlug(slug: string): boolean {
  return /^[a-z][a-z0-9_]{1,253}$/.test(slug);
}

function validateRiskTier(tier: string): boolean {
...
export function createActionTypesRouter(): Router {
  const router = Router();
  ...
  router.post('/validate', async (req: Request, res: Response) => {
    try {
      ...
      const actionType = await queryOne<ActionType>(
  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      ...
      const rows = await query<ActionType & { usage_count: number }>(sql, params);
  ...
  router.get('/:id', async (req: Request, res: Response) => {
    try {
      ...
      const actionType = await queryOne<ActionType>(
        'SELECT * FROM action_types WHERE id = $1',
        [id]
      );
      ...
      const usageStats = await query<{ day: string; count: string }>(
        `SELECT DATE(executed_at) as day, COUNT(*)::text as count
      ...
      const totalUsage = await queryOne<{ count: string }>(
        'SELECT COUNT(*)::text as count FROM action_type_usage WHERE action_type_id = $1',
        [id]
      );
  ...
  router.post('/', async (req: Request, res: Response) => {
    try {
      const body: CreateActionTypeBody = req.body;
      ...
      const created = await queryOne<ActionType>(
        `INSERT INTO action_types (action_type, display_name, description, category, payload_schema, default_risk_tier, is_builtin, icon, color, created_by)
  ...
  router.put('/:id', async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const body: UpdateActionTypeBody = req.body;

      const existing = await queryOne<ActionType>(
        'SELECT * FROM action_types WHERE id = $1',
        [id]
      );
      ...
      if (existing.is_builtin) {
        if (body.enabled !== undefined) {
          await execute(
            'UPDATE action_types SET enabled = $1, updated_at = NOW() WHERE id = $2',
            [body.enabled, id]
          );

          const updated = await queryOne<ActionType>(
            'SELECT * FROM action_types WHERE id = $1',
            [id]
          );
      ...
      const updated = await queryOne<ActionType>(
        `UPDATE action_types SET ${sets.join(', ')} WHERE id = $${paramIdx} RETURNING *`,
        params
      );
  ...
  router.get('/:id/usage', async (req: Request, res: Response) => {
    try {
      ...
      const existing = await queryOne<ActionType>(
        'SELECT id FROM action_types WHERE id = $1',
        [id]
      );
      ...
      const rows = await query<ActionTypeUsage>(
        `SELECT * FROM action_type_usage
