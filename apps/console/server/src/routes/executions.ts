...
export function createExecutionsRouter(vienna: ViennaRuntimeService): Router {
  const router = Router();
  ...
  router.get('/:id', async (req: Request, res: Response) => {
    try {
      ...
      let plan = null;
      if (summary.plan_id) {
        const plans = await stateGraph.query(
      ...
      if (summary.verification_id) {
        const verifications = await stateGraph.query(
      ...
      if (summary.outcome_id) {
        const outcomes = await stateGraph.query(
          'SELECT * FROM workflow_outcomes WHERE outcome_id = ?',
          [summary.outcome_id]
        );
      ...
      const timeline = await stateGraph.query(
        'SELECT event_type, stage, event_timestamp, summary, status FROM execution_ledger_events WHERE execution_id = ? ORDER BY sequence_num ASC',
        [id]
      );
