...
export function createIntegrationsRouter(): Router {
  ...
  router.get('/:id', async (req: Request, res: Response) => {
    try {
      const integration = await queryOne<IntegrationRecord>(
        `SELECT i.*, 
          (SELECT COUNT(*) FROM integration_events ie WHERE ie.integration_id = i.id) as event_count
  ...
  router.post('/', async (req: Request, res: Response) => {
    try {
      ...
      const integration = await queryOne<IntegrationRecord>(
        `INSERT INTO integrations (type, name, description, config, event_types, filters)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [type, name, description || null, JSON.stringify(config || {}), JSON.stringify(event_types || ['approval_required']), JSON.stringify(filters || {})]
      );
  ...
  router.put('/:id', async (req: Request, res: Response) => {
    try {
      const existing = await queryOne<IntegrationRecord>('SELECT * FROM integrations WHERE id = $1', [req.params.id]);
      if (!existing) {
        res.status(404).json({ success: false, error: 'Integration not found', code: 'NOT_FOUND', timestamp: new Date().toISOString() });
        return;
      }
      ...
      const updated = await queryOne<IntegrationRecord>(
  ...
  router.post('/:id/test', async (req: Request, res: Response) => {
    try {
      const integration = await queryOne<IntegrationRecord>('SELECT * FROM integrations WHERE id = $1', [req.params.id]);
      if (!integration) {
        res.status(404).json({ success: false, error: 'Integration not found', code: 'NOT_FOUND', timestamp: new Date().toISOString() });
        return;
      }
  ...
  router.post('/:id/toggle', async (req: Request, res: Response) => {
    try {
      ...
      const updated = await queryOne<IntegrationRecord>(
        `UPDATE integrations SET enabled = NOT enabled, updated_at = NOW() WHERE id = $1 RETURNING *`,
        [req.params.id]
      );
  ...
  router.get('/:id/events', async (req: Request, res: Response) => {
    try {
      ...
      const events = await query<IntegrationEventRecord>(
        `SELECT * FROM integration_events WHERE integration_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
        [req.params.id, limit, offset]
      );

      const total = await queryOne<{ count: string }>(
        'SELECT COUNT(*) as count FROM integration_events WHERE integration_id = $1',
        [req.params.id]
      );
  ...
  // ──────────────────────────────────────────────
  router.post('/callbacks/slack', async (req: Request, res: Response) => {
    try {
  ...
  // ──────────────────────────────────────────────
  router.get('/callbacks/email', async (req: Request, res: Response) => {
    try {
      ...
      const emailIntegration = await queryOne<IntegrationRecord>(
        `SELECT * FROM integrations WHERE type = 'email' AND enabled = true LIMIT 1`
      );
      const secret = emailIntegration?.config?.signing_secret || emailIntegration?.config?.api_key || 'vienna-default-secret';

      const verification = verifyApprovalToken(token as string, secret);
...
function maskConfig(integration: any): any {
...
function renderCallbackPage(title: string, message: string, success: boolean): string {
