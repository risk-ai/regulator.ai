...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const policies = await query<Policy>(
      `SELECT id, name, description, enabled, priority, created_at, updated_at
       FROM policies
       WHERE tenant_id = $1
    ...
    const policiesWithRules = await Promise.all(
      policies.map(async (policy) => {
        const rules = await query<PolicyRule>(
          `SELECT id, rule_type, conditions, action, enabled
           FROM policy_rules
           WHERE policy_id = $1
           ORDER BY id`,
          [policy.id]
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const policy = await queryOne<Policy>(
      `INSERT INTO policies (tenant_id, name, description, enabled, priority)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, tenant_id, name, description, enabled, priority, created_at`,
      [tenantId, name, description || null, enabled, priority]
    );
    ...
    for (const rule of rules) {
      ...
      const createdRule = await queryOne<PolicyRule>(
        `INSERT INTO policy_rules (policy_id, rule_type, conditions, action, enabled)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, rule_type, conditions, action, enabled`,
        [policy.id, rule_type, JSON.stringify(conditions), action, ruleEnabled]
      );
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const existing = await queryOne<Policy>(
    ...
    // Update policy
    const updated = await queryOne<Policy>(
      `UPDATE policies
       ...
       RETURNING id, name, description, enabled, priority, updated_at`,
      [name, description, enabled, priority, id, tenantId]
    );
