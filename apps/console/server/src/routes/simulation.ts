...
router.post('/run', async (req: Request, res: Response) => {
  try {
    ...
    switch (scenario) {
      case 'flood_intents':
        result = await engine.floodIntents(
          config.agent_id || `chaos_agent_${Date.now()}`,
          config.count || 20,
          config.interval || 100
        );
        break;
      ...
      case 'scope_creep':
        result = await engine.scopeCreep(
          config.agent_id || `chaos_agent_${Date.now()}`,
          config.escalating_actions
        );
        break;

      case 'budget_exhaust':
        result = await engine.budgetExhaust(
          config.agent_id || `chaos_agent_${Date.now()}`,
          config.amount || 1000
        );
        break;
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      success: true,
      simulations: simulations.map((s: any) => s.toJSON()),
 ...
 */
router.get('/scenarios/list', async (req: Request, res: Response) => {
  try {
    const scenarios = [
      {
        name: 'flood_intents',
