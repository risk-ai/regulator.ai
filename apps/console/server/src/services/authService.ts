...
export interface AuthConfig {
  ...
  stateGraph?: any; // SQLite database handle
}
...
export class AuthService {
  ...
  async register(params: {
    ...
    company?: string;
    plan?: string;
  }): Promise<{ operator: Operator; session: Session } | { error: string }> {
    ...
    if (this.dbReady && this.config.stateGraph?.run) {
      try {
        await this.config.stateGraph.run(
          `INSERT INTO operators (id, username, email, company, password_hash, tenant_id, plan, created_at, active)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`,
          [id, username, email || null, company || null, passwordHash, tenantId, plan || 'community', now]
        );
      } catch (err: any) {
  ...
  async login(password: string, username?: string): Promise<Session | null> {
  ...
  private async loginWithOperator(operator: Operator): Promise<Session> {
    return this.createSession(operator.username, operator.tenantId);
  }

  private createSession(operator: string, tenantId: string): Session {
    ...
    if (this.dbReady && this.config.stateGraph?.run) {
      this.config.stateGraph.run(
        `INSERT OR REPLACE INTO sessions (session_id, operator_username, tenant_id, created_at, expires_at, last_activity)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [sessionId, operator, tenantId, session.createdAt, session.expiresAt, session.lastActivity]
      ).catch(() => {}); // Non-blocking
    }
  ...
  async createApiKey(operatorId: string, label: string): Promise<{ key: string; apiKey: ApiKey } | null> {
    // Find operator
    let operator: Operator | undefined;
    for (const op of this.operators.values()) {
      if (op.id === operatorId) { operator = op; break; }
    }
    ...
    if (this.dbReady && this.config.stateGraph?.run) {
      await this.config.stateGraph.run(
        `INSERT INTO api_keys (id, key_hash, operator_id, tenant_id, label, created_at, active)
         VALUES (?, ?, ?, ?, ?, ?, 1)`,
        [id, keyHash, operatorId, operator.tenantId, label, now]
      ).catch(() => {});
    }
  ...
  async validateApiKey(rawKey: string): Promise<{ tenantId: string; operatorId: string } | null> {
    const keyHash = crypto.createHash('sha256').update(rawKey).digest('hex');
  ...
  async validateSession(sessionId: string): Promise<Session | null> {
    ...
    if (!session && this.dbReady && this.config.stateGraph?.get) {
      try {
        const row = await this.config.stateGraph.get(
  ...
  getOperatorCount(): number { return this.operators.size; }
  clearAllSessions(): void { this.sessions.clear(); }
}
