...
function generateApprovalToken(approvalId: string, action: 'approve' | 'deny', secret: string): string {
  const payload = {
    aid: approvalId,
    act: action,
...
export function verifyApprovalToken(token: string, secret: string): { valid: boolean; payload?: any; error?: string } {
  try {
    const [data, sig] = token.split('.');
    if (!data || !sig) return { valid: false, error: 'Malformed token' };
...
function buildEmailHtml(event: IntegrationEvent, config: Record<string, any>): string {
...
async function sendViaResend(config: Record<string, any>, subject: string, html: string): Promise<{ success: boolean; response?: any; error?: string }> {
  try {
...
async function sendViaSmtp(config: Record<string, any>, subject: string, html: string): Promise<{ success: boolean; response?: any; error?: string }> {
...
export const emailAdapter: IntegrationAdapter = {
  ...
  validateConfig(config) {
    ...
    if (provider === 'resend') {
      if (!config.api_key) errors.push('Resend API key is required');
      if (!config.from) errors.push('From address is required');
      if (!config.to || (Array.isArray(config.to) && config.to.length === 0)) errors.push('At least one recipient is required');
    } else {
      if (!config.smtp_host) errors.push('SMTP host is required');
      if (!config.smtp_user) errors.push('SMTP user is required');
      if (!config.smtp_pass) errors.push('SMTP password is required');
      if (!config.to_addresses) errors.push('At least one recipient is required');
    }
