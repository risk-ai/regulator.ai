...
import type { ProviderHealth as LibProviderHealth, ProviderHealthTransition } from '@vienna-lib/providers/types.js';
...
/**
 * Execution record for health calculation
 */
...
export class ProviderHealthService {
  ...
  private healthTransitions: Map<string, ProviderHealthTransition[]> = new Map();
  private activeExecutions: Map<string, number> = new Map();
  
  private maxHistoryPerProvider = 1000;
  private maxTransitionsPerProvider = 50;
  private staleTelemetryThresholdMs = 5 * 60 * 1000; // 5 minutes
  
  constructor(private providerManager?: ProviderManagerBridge) {
   ...
   */
  async getProvidersHealth(): Promise<ProvidersHealthSnapshot> {
   ...
   */
  async getProviderHealth(providerName: string): Promise<ProviderHealthDetail | null> {
    try {
      let baseStatus: any = null;
  ...
  private calculateProviderHealth(
    ...
    if (previousStatus && previousStatus !== status) {
      this.recordTransition(providerName, previousStatus, status, 
        this.getTransitionReason(previousStatus, status, successRate, requestCount));
    }
   ...
   */
  private getCurrentStatus(provider: string): ProviderHealthState | null {
