...
export class ReconciliationService {
  ...
  private async ensureStateGraph() {
    if (!this.getStateGraph) {
      const module = await import('@vienna-lib/state/state-graph.js');
   ...
   */
  async getExecutionLeases(): Promise<ExecutionLease[]> {
   ...
   */
  async getTimeline(limit: number = 100): Promise<{ events: TimelineEvent[]; total: number }> {
    const stateGraph = await this.ensureStateGraph();
    ...
    // Get total count
    const countResult = await stateGraph.query(
      'SELECT COUNT(*) as total FROM managed_objective_history'
    );
  ...
  async getMetrics(): Promise<ReconciliationMetrics> {
    ...
    if (durationRows.length > 0) {
      const durations = durationRows.map((r: any) => r.duration_ms);
   ...
   */
  private mapEventType(transitionType: string): string {
   ...
   */
  private generateEventSummary(row: any): string {
   ...
   * Enable safe mode
   */
  async enableSafeMode(reason: string, operator: string) {
    await this.ensureStateGraph();
    const stateGraph = this.getStateGraph();
    await stateGraph.initialize();
   ...
   * Disable safe mode
   */
  async disableSafeMode(operator: string) {
    await this.ensureStateGraph();
    const stateGraph = this.getStateGraph();
    await stateGraph.initialize();
