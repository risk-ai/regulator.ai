...
class RegionService {
   ...
   */
  public routeToNearestRegion(req: Request, res: Response): string | null {
    const flyRegion = req.headers['fly-region'] as string;
    const clientRegion = req.headers['cf-ipcountry'] as string;
   ...
   */
  public validateDataResidency(tenantId: string, requestedRegion: string): boolean {
   ...
   */
  public getDatabaseUrl(regionId: string): string {
   ...
   */
  public recordRequest(regionId: string, latencyMs: number, isError: boolean = false): void {
    const metrics = this.metrics.get(regionId);
    if (!metrics) {
   ...
   */
  public getMetrics(): Map<string, RegionMetrics> {
   ...
   */
  public getRegions(): Map<string, Region> {
  ...
  private getRegionByFlyCode(flyCode: string): Region | undefined {
  ...
  private selectTargetRegion(clientCountry?: string, currentFlyRegion?: string): Region | undefined {
  ...
  private getTenantRegion(tenantId: string): { dataResidencyRegion?: string; requiresGDPR: boolean } | null {
...
export function regionMiddleware(req: Request, res: Response, next: any) {
  ...
  try {
    ...
    (req as any).region = targetRegion;
