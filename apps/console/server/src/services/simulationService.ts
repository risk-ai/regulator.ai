...
interface AgentProfile {
  agentId: string;
  actionTypes: { type: string; riskTier: string; weight: number }[];
  actionsPerDay: [number, number]; // [min, max]
...
const ALERT_TEMPLATES = [
  {
    ...
    severity: 'warning',
    messageTemplate: (agentId: string, actionType: string) =>
      `Agent ${agentId} attempted ${actionType} action outside permitted scope`,
  },
  {
    type: 'rate_limit',
    severity: 'warning',
    messageTemplate: (agentId: string, _actionType: string) =>
      `Agent ${agentId} approaching rate limit threshold (${80 + Math.floor(Math.random() * 18)}% of hourly limit)`,
  },
  {
    type: 'anomaly',
    severity: 'info',
    messageTemplate: (agentId: string, _actionType: string) =>
      `Unusual activity pattern detected for ${agentId}: volume ${(2 + Math.random() * 3).toFixed(1)}x above baseline`,
  },
  {
    type: 'trust_decay',
    severity: 'critical',
    messageTemplate: (agentId: string, _actionType: string) =>
      `Trust score for ${agentId} declining due to elevated error rate`,
  },
];
...
function businessHoursMultiplier(hour: number): number {
...
function generateLatency(riskTier: string): number {
  switch (riskTier) {
...
function generateActionContext(actionType: string, riskTier: string): Record<string, any> {
  const ctx: Record<string, any> = { source: 'simulation' };
  ...
  switch (actionType) {
    case 'financial_transaction': {
      ...
      if (highValue) ctx.risk_tier_override = 'T2';
      break;
    }
    case 'deploy': {
...
class SimulationService {
  ...
  private cachedPolicyRules: PolicyRuleRow[] = [];
  private lastPolicyRefresh = 0;
  private readonly POLICY_REFRESH_INTERVAL = 60000; // Refresh rules every 60s
  ...
  async seed(): Promise<{ actions: number; alerts: number }> {
    ...
    for (let hoursAgo = 24; hoursAgo >= 0; hoursAgo--) {
      ...
      for (const profile of AGENT_PROFILES) {
        ...
        for (let i = 0; i < actionsThisHour; i++) {
          ...
          const actionDef = weightedRandom(
            profile.actionTypes.map(a => ({ item: a, weight: a.weight }))
          );
          ...
          try {
            await execute(
              ...
              [profile.agentId, actionDef.type, result, latency, riskTier, JSON.stringify(context), actionTimestamp.toISOString()]
            );
            totalActions++;
            ...
            if (Math.random() < 0.02) {
              ...
              await execute(
                `INSERT INTO agent_alerts (agent_id, alert_type, severity, message, context, resolved, resolved_by, resolved_at, created_at)
    ...
    for (const profile of AGENT_PROFILES) {
      ...
      await execute(
        `UPDATE agent_registry SET last_heartbeat = NOW() - INTERVAL '${minutesAgo} minutes' WHERE agent_id = $1`,
        [profile.agentId]
      ).catch(() => {});
    }
  ...
  private async tick(): Promise<void> {
    try {
      ...
      for (const profile of AGENT_PROFILES) {
        ...
        const actionsThisTick = Math.min(
          Math.round(tickRate * burstMultiplier + (Math.random() < tickRate % 1 ? 1 : 0)),
          3 // Cap at 3 per tick per agent
        );
        ...
        if (actionsThisTick > 0 || Math.random() < 0.3) {
          await execute(
            `UPDATE agent_registry SET last_heartbeat = NOW() WHERE agent_id = $1`,
            [profile.agentId]
          ).catch(() => {});
        }
      }
  ...
  private async generateAction(profile: AgentProfile, timestamp: Date): Promise<void> {
    const actionDef = weightedRandom(
      profile.actionTypes.map(a => ({ item: a, weight: a.weight }))
    );
    ...
    try {
      await execute(
        `INSERT INTO agent_activity (agent_id, action_type, result, latency_ms, risk_tier, error_message, context, created_at)
      ...
      if (policyResult.matchedRuleId) {
        await execute(
          `INSERT INTO policy_evaluations (rule_id, agent_id, action_type, result, action_taken, context_snapshot, evaluated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`,
          [
  ...
  private evaluateAgainstPolicies(
    ...
    riskTier: string
  ): { outcome: string; matchedRuleId: string | null; actionTaken: string | null } {
  ...
  private determineOutcome(): string {
    const r = Math.random();
    if (r < 0.85) return 'executed';
    if (r < 0.95) return 'denied';
    return 'failed';
  }

  private async adjustTrustScore(agentId: string, result: string): Promise<void> {
    let delta = 0;
    if (result === 'executed') delta = 1;
    else if (result === 'denied') delta = -2;
    else if (result === 'failed') delta = -5;
  ...
  private async generateAlert(agentId: string, actionType: string, timestamp: Date): Promise<void> {
    const template = ALERT_TEMPLATES[randomInt(0, ALERT_TEMPLATES.length - 1)];

    try {
      await execute(
        `INSERT INTO agent_alerts (agent_id, alert_type, severity, message, context, created_at)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [
          agentId,
          template.type,
  ...
  private async trackActionTypeUsage(
    actionType: string,
    agentId: string,
    status: string,
    timestamp: Date
    ...
    try {
      const at = await queryOne<{ id: string }>(
        `SELECT id FROM action_types WHERE action_type = $1`,
        [actionType]
      );
      if (at) {
        await execute(
          `INSERT INTO action_type_usage (action_type_id, agent_id, status, executed_at)
           VALUES ($1, $2, $3, $4)`,
          [at.id, agentId, status, timestamp.toISOString()]
        );
      }
    } catch {
      // Skip — non-critical
  ...
  private async refreshPolicyRules(): Promise<void> {
    try {
      this.cachedPolicyRules = await query<PolicyRuleRow>(
        `SELECT id, name, conditions, action_on_match, approval_tier, enabled, priority
  ...
  private async seedPolicyRules(): Promise<void> {
    try {
      ...
      for (const rule of DEMO_POLICY_RULES) {
        await execute(
          `INSERT INTO policy_rules (name, description, conditions, action_on_match, approval_tier, priority, enabled, created_by)
