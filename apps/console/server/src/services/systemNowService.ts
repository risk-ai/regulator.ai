...
import type { SystemState } from '../types/api.js';

/**
 * Configuration for attention thresholds
 */
interface AttentionConfig {
  queueCapacityThreshold: number; // Queue depth warning threshold
  stalledExecutionMs: number; // Execution time before flagged as stalled
  retryLoopThreshold: number; // Retry attempts before flagged
  deadLetterGrowthWindow: number; // Time window for growth detection (ms)
  failureRateThreshold: number; // Failure rate % before alerting
}
...
export class SystemNowService {
  ...
  constructor(
    ...
    private runtimeStats: RuntimeStatsService,
    private providerHealth?: ProviderHealthService,
   ...
   */
  getRecentEventsFiltered(limit: number = 50, failuresOnly: boolean = false): ActivityEvent[] {
   ...
   */
  private deriveSystemState(status: any): SystemState {
    // Simple heuristic - can be enhanced
    if (status.paused) return 'degraded';
    if (status.blocked > 10) return 'degraded';
    if (status.deadLetterCount > 20) return 'degraded';
    return 'healthy';
  }
   ...
   */
  private async getQueueHealth(): Promise<QueueHealthSnapshot> {
    try {
   ...
   */
  private async getCurrentWork(): Promise<CurrentWorkItem[]> {
    try {
   ...
   * Get recent failures
   */
  private async getRecentFailures(): Promise<RecentFailures> {
    try {
      const stats = await this.runtimeStats.getRuntimeStats('5m');
      ...
      const uniqueEnvelopes = new Set<string>();
   ...
   */
  private async getDeadLetterStats(): Promise<{
    count: number;
    recentCount: number;
    growing: boolean;
  }> {
  ...
  private async getAttentionItems(): Promise<AttentionItem[]> {
    ...
    try {
      ...
      if (providerHealth.degraded > 0 || providerHealth.unavailable > 0) {
        const degradedProviders = providerHealth.providers.filter(
          p => p.state === 'degraded' || p.state === 'unavailable'
        );
