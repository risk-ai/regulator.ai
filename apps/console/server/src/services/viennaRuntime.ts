...
export class ViennaRuntimeService {
  private viennaCore: any; // Vienna Core instance (CommonJS)
  private providerManager: any; // ProviderManagerBridge instance
  private replayService: any; // ReplayService instance (lazy loaded)
  private deadLetterQueue: any; // Dead Letter Queue instance
  ...
  private async getReplayService(): Promise<any> {
  ...
  async getSystemStatus(): Promise<SystemStatus> {
    try {
      ...
      const health: any = this.viennaCore.queuedExecutor.getHealth();
      const controlState: any = this.viennaCore.queuedExecutor.getExecutionControlState();
      const queueState: any = this.viennaCore.queuedExecutor.getQueueState();
      ...
      const healthStateMap: Record<string, any> = {
        'HEALTHY': 'healthy',
        'WARNING': 'degraded',
        'CRITICAL': 'critical'
      };
      ...
      return {
        ...
        integrity_state: 'ok' as any,
        trading_guard_state: (tradingState.guard_active ? 'active' : 'disabled') as any,
  ...
  async getObjective(objectiveId: string): Promise<ObjectiveDetail | null> {
    try {
      ...
      const matchingEnvelopes = allEnvelopes.filter((e: any) => e.objective_id === objectiveId);
      ...
      return {
        ...
        active_count: matchingEnvelopes.filter((e: any) => e.state === 'executing').length,
        blocked_count: matchingEnvelopes.filter((e: any) => e.state === 'blocked').length,
        dead_letter_count: 0,
        completed_count: 0,
        current_step: firstEnvelope.description || undefined,
        current_envelope_id: matchingEnvelopes.find((e: any) => e.state === 'executing')?.envelope_id,
  ...
  private normalizeObjectiveDetail(raw: any): ObjectiveDetail {
    return {
  ...
  private inferObjectiveStatus(envelopes: any[]): string {
    if (envelopes.some((e: any) => e.state === 'failed' || e.state === 'dead_letter')) {
      return 'failed';
    }
    if (envelopes.some((e: any) => e.state === 'blocked')) {
      return 'blocked';
    }
    if (envelopes.every((e: any) => e.state === 'completed')) {
      return 'completed';
    }
    if (envelopes.some((e: any) => e.state === 'executing')) {
      return 'executing';
    }
    return 'pending';
  }
  ...
  async getObjectiveCausalChain(objectiveId: string): Promise<{
   ...
   */
  async getObjectiveTrackerStats(): Promise<any> {
    try {
      if (!this.viennaCore?.queuedExecutor?.objectiveTracker) {
        return {
          total_objectives: 0,
   ...
   */
  async getEnvelopeLineage(envelopeId: string): Promise<any[]> {
    try {
   ...
   */
  async validateLineage(): Promise<any> {
    try {
      if (!this.viennaCore?.queuedExecutor?.lineageValidator) {
        return { 
          valid: true, 
   ...
   */
  async getExecutingEnvelopes(): Promise<any[]> {
    try {
      ...
      return executingEntries.map((entry: any) => ({
  ...
  async getRecentFailures(limit: number = 20): Promise<any[]> {
    try {
      ...
      return dlqEntries.map((entry: any) => ({
        envelopeId: entry.envelope_id,
  ...
  async resumeExecution(request: ResumeExecutionRequest): Promise<{
    resumed_at: string;
  ...
  async cancelDeadLetter(
    envelopeId: string,
    request: CancelDeadLetterRequest
  ...
  async queryAudit(params: any): Promise<{
    records: any[];
    total: number;
    has_more: boolean;
  }> {
  ...
  async getAuditRecord(auditId: string): Promise<any | null> {
    try {
  ...
  async getServices(): Promise<Array<{
    ...
    try {
      ...
      if (this.viennaCore.stateAwareDiagnostics) {
        ...
        return serviceStatuses.map((s: any) => ({
          service: s.service_id,
          status: s.status as 'running' | 'degraded' | 'stopped' | 'unknown',
          lastHeartbeatAt: s.last_check_at,
          connectivity: s.metadata?.connectivity as 'healthy' | 'degraded' | 'offline',
      ...
      try {
        ...
        services.push({
          ...
          status: (healthy ? 'running' : 'degraded') as 'running' | 'degraded',
          lastHeartbeatAt: new Date().toISOString(),
          connectivity: (healthy ? 'healthy' : 'degraded') as 'healthy' | 'degraded',
          restartable: true
        });
      } catch (error) {
        ...
        services.push({
          ...
          connectivity: 'offline' as 'offline',
          restartable: true
        });
      }
      ...
      try {
        ...
        services.push({
          ...
          status: (health.executor_ready ? 'running' : 'degraded') as 'running' | 'degraded',
          lastHeartbeatAt: new Date().toISOString(),
          connectivity: (health.queue_healthy ? 'healthy' : 'degraded') as 'healthy' | 'degraded',
    ...
    try {
      ...
      if (this.viennaCore.serviceManager) {
        ...
        return {
          ...
          status: result.status as 'preview' | 'executing' | 'failed',
   ...
   */
  private async generateEnvelopesFromPlan(
    plan: any,
    operator: string
  ): Promise<any[]> {
    const envelopes: any[] = [];
    let previousEnvelopeId: string | undefined;
  ...
  async getRuntimeEnvelope(envelopeId: string): Promise<{
    envelope_id: string;
    objective_id: string;
    parent_envelope_id?: string;
    action_type: string;
  ...
  async getObjectiveExecution(objectiveId: string): Promise<{
    objective_id: string;
    title: string;
    status: string;
    envelopes: Array<{
    ...
    try {
      ...
      function buildTree(parentId: string, depth: number) {
   ...
   */
  async processRecoveryIntent(message: string): Promise<string> {
    try {
   ...
   */
  async forceRuntimeMode(mode: string, reason: string): Promise<{
    from: string;
    to: string;
    timestamp: string;
    reason: string;
  ...
  classifyChatIntent(message: string): 'recovery' | 'general' {
    ...
    return this.viennaCore.classifyChatIntent(message) as 'recovery' | 'general';
  }
   ...
   */
  async executeSystemCommand(commandName: string, args: any[] = [], context: any = {}): Promise<any> {
    try {
   ...
   */
  async diagnoseAndProposeFixes(): Promise<any> {
    try {
   ...
   */
  async approveAndExecuteT1(action: any, approver: string): Promise<any> {
    ...
    try {
      ...
      const result = await this.viennaCore.sendOpenClawDirection(
  ...
  async denyT1Action(action: any, reason: string): Promise<void> {
