...
export class WorkspaceResolver {
  ...
  private fileIndex: Map<string, string[]> = new Map(); // filename → [paths]
  private lastIndexed: number = 0;
  private indexTTL: number = 30000; // 30 seconds
   ...
   */
  async resolve(query: string): Promise<FileResolutionResult> {
    // Ensure fresh index
    ...
    const matches: Array<{ fileName: string; path: string; relativePath: string }> = [];
