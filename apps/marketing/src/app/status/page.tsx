  ...
  return (
    <div className="min-h-screen bg-[#0a0e14]">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        {/* Services */}
        <h2 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-3">Services</h2>
        <div className="space-y-2 mb-8">
          {services.map((svc) => (
            <div key={svc.name} className="flex items-center justify-between bg-black border border-zinc-800 px-4 py-3">
              <div>
                <span className="text-sm text-white font-medium">{svc.name}</span>
                <span className="text-xs text-zinc-600 ml-3 font-mono">{svc.endpoint}</span>
              </div>
          ...
          {services.length === 0 && !loading && (
            <div className="text-center text-sm text-zinc-500 py-4">No service data yet. Refreshing...</div>
          )}
        </div>

        {/* Uptime */}
        {uptime && (
          <div className="bg-black border border-zinc-800 p-6 text-center mb-8">
            <div className="text-3xl font-bold text-white font-mono mb-1">{formatUptime(uptime)}</div>
            <div className="text-xs text-zinc-500">Current uptime</div>
          </div>
        )}

        <p className="text-center text-xs text-zinc-600">
          For incidents or outages, contact <a href="mailto:admin@ai.ventures" className="text-amber-500">admin@ai.ventures</a>
        </p>
      </main>
    </div>
  );
