...
exports.AgentTrustEngine = void 0;
// ─── Trust Score Engine ───
class AgentTrustEngine {
    ...
    computeScore(agentId, tenantId, events, previousScores) {
        ...
        const score = Math.round(approvalRate.score +
    ...
    computeAll(tenantId, events, previousScores) {
        ...
        return Array.from(agentIds).map(agentId => this.computeScore(agentId, tenantId, events, previousScores?.get(agentId)));
    }
    // ─── Component Computations ───
    _computeApprovalRate(events) {
        const intentEvents = events.filter(e => e.event?.includes('approved') || e.event?.includes('denied') || e.event?.includes('submitted'));
        ...
        if (total === 0)
            return { score: 15, max: 30, approved: 0, total: 0 }; // Neutral
    ...
    _computeComplianceRate(events) {
        const scopeEvents = events.filter(e => e.in_scope !== undefined || e.event?.includes('scope'));
        const inScope = scopeEvents.filter(e => e.in_scope || !e.event?.includes('drift')).length;
        const total = scopeEvents.length;
        if (total === 0)
            return { score: 12, max: 25, in_scope: 0, total: 0 }; // Neutral
    ...
    _computeStability(events) {
        ...
        if (counts.length <= 1)
            return { score: 10, max: 20, variance: 0 }; // Neutral
    ...
    _computeAnomalyFactor(events) {
        ...
        const totalActions = events.filter(e => e.event?.includes('submitted') || e.event?.includes('intent')).length;
        if (totalActions === 0)
            return { score: 8, max: 15, anomalies: 0, total_actions: 0 }; // Neutral
    ...
    _computeTenure(events) {
        if (events.length === 0)
            return { score: 0, max: 10, days_active: 0 };
    ...
    _generateRecommendation(score, level, components, events) {
        ...
        for (const e of events) {
            if (e.risk_tier)
                tierCounts[e.risk_tier] = (tierCounts[e.risk_tier] || 0) + 1;
        }
        ...
        if (level === 'watch') {
            const issues = [];
            if (components.approval_rate.score < 15)
                issues.push('low approval rate');
            if (components.anomaly_factor.score < 8)
                issues.push('elevated anomaly rate');
            if (components.stability.score < 10)
                issues.push('inconsistent activity pattern');
            return {
                action: 'review',
...
exports.AgentTrustEngine = AgentTrustEngine;
exports.default = AgentTrustEngine;
