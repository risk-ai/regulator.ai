...
var natural_language_policy_builder_exports = {};
__export(natural_language_policy_builder_exports, {
  NaturalLanguagePolicyBuilder: () => NaturalLanguagePolicyBuilder,
  default: () => natural_language_policy_builder_default
});
module.exports = __toCommonJS(natural_language_policy_builder_exports);
const DECISION_KEYWORDS = {
  ...
  // "block X without approval" → require approval
};
...
class NaturalLanguagePolicyBuilder {
  /**
  ...
  parse(input) {
    ...
    const stopWords = /* @__PURE__ */ new Set(["a", "an", "the", "for", "from", "to", "in", "on", "of", "any", "all", "no", "not", "and", "or", "with", "by", "over", "under", "above", "below", "than", "that", "this", "it", "is", "are", "be", "been", "being", "do", "does", "did", "will", "would", "could", "should", "may", "might", "must", "shall", "can", "making", "agent", "agents", "operations", "operation", "action", "actions", "automatically", "only"]);
  ...
  // ─── Helpers ───
  _tokenize(input) {
    return input.replace(/[^a-z0-9\s$.,*-]/g, " ").split(/\s+/).filter((t) => t.length > 0);
  }
  _computeConfidence(matched, unmatched, decision, actions) {
    let confidence = 30;
    if (decision) confidence += 25;
    if (actions.length > 0) confidence += 25;
    confidence += Math.min(20, matched.length * 5);
    const stopWords = /* @__PURE__ */ new Set(["a", "an", "the", "for", "from", "to", "in", "on", "of", "any", "all", "and", "or", "with", "by", "agent", "agents", "operations", "automatically", "only"]);
  ...
  _generatePolicyName(decision, actions, environment) {
  ...
  _generateExplanation(policy, matched) {
    const parts = [];
    ...
    if (policy.constraints) {
      const cstParts = Object.entries(policy.constraints).map(([key, val]) => {
        if (val.max !== void 0) return `${key} \u2264 ${val.max}`;
        if (val.min !== void 0) return `${key} \u2265 ${val.min}`;
        return key;
      });
...
var natural_language_policy_builder_default = NaturalLanguagePolicyBuilder;
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  NaturalLanguagePolicyBuilder
});
