...
exports.OpenWarrantStandard = void 0;
...
class OpenWarrantStandard {
    ...
    _validateClaims(payload) {
        if (!payload.wid)
            throw new Error('wid (warrant ID) is required');
        if (!payload.iss)
            throw new Error('iss (issuer) is required');
        if (!payload.sub)
            throw new Error('sub (subject/agent) is required');
        if (!payload.aud)
            throw new Error('aud (audience) is required');
        if (!payload.exp)
            throw new Error('exp (expiration) is required');
        if (!payload.tier)
            throw new Error('tier (risk tier) is required');
        if (!payload.scope || payload.scope.length === 0)
            throw new Error('scope (allowed actions) is required');
        if (!payload.obj)
            throw new Error('obj (objective) is required');
        if (!payload.pid)
            throw new Error('pid (plan ID) is required');
        ...
        if (payload.tier === 'T3') {
            if (!payload.jst)
                throw new Error('T3 warrants require jst (justification)');
            if (!payload.rbk)
                throw new Error('T3 warrants require rbk (rollback plan)');
    ...
    _sign(input, key) {
        ...
        if (!alg)
            throw new Error(`Unsupported algorithm: ${key.alg}. Currently supported: HS256, HS384, HS512`);
        if (!key.secret)
            throw new Error(`Key ${key.kid} has no secret for HMAC signing`);
...
exports.OpenWarrantStandard = OpenWarrantStandard;
exports.default = OpenWarrantStandard;
