...
exports.PostgresWarrantChainStore = exports.InMemoryWarrantChainStore = void 0;
...
class InMemoryWarrantChainStore {
    ...
    async appendWarrant(warrant) {
        ...
        if (chain.length > 0 && warrant.chain_index !== chain.length) {
            throw new Error(`Chain append violation: expected index ${chain.length}, got ${warrant.chain_index}`);
        }
        chain.push({ ...warrant });
...
exports.InMemoryWarrantChainStore = InMemoryWarrantChainStore;
...
class PostgresWarrantChainStore {
    ...
    async appendWarrant(warrant) {
        await this.execute(`INSERT INTO warrant_chain 
    ...
    async getLatest(tenantId) {
        const rows = await this.query(`SELECT * FROM warrant_chain WHERE tenant_id = $1 ORDER BY chain_index DESC LIMIT 1`, [tenantId]);
        return rows.length > 0 ? this._rowToWarrant(rows[0]) : null;
    }
    async getByIndex(tenantId, index) {
        const rows = await this.query(`SELECT * FROM warrant_chain WHERE tenant_id = $1 AND chain_index = $2`, [tenantId, index]);
        return rows.length > 0 ? this._rowToWarrant(rows[0]) : null;
    }
    async getByWarrantId(warrantId) {
        const rows = await this.query(`SELECT * FROM warrant_chain WHERE warrant_id = $1`, [warrantId]);
        return rows.length > 0 ? this._rowToWarrant(rows[0]) : null;
    }
    async getChain(tenantId, fromIndex = 0, toIndex) {
    ...
    async getChainLength(tenantId) {
        const rows = await this.query(`SELECT COUNT(*) as count FROM warrant_chain WHERE tenant_id = $1`, [tenantId]);
        return parseInt(rows[0]?.count || '0', 10);
    }
    async saveAnchor(anchor) {
        await this.execute(`INSERT INTO warrant_chain_anchors 
    ...
    async getLatestAnchor(tenantId) {
        const rows = await this.query(`SELECT * FROM warrant_chain_anchors WHERE tenant_id = $1 ORDER BY anchored_at DESC LIMIT 1`, [tenantId]);
        if (rows.length === 0)
            return null;
        const row = rows[0];
        return {
...
exports.PostgresWarrantChainStore = PostgresWarrantChainStore;
