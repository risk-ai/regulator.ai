...
exports.InMemoryDelegationStore = exports.WarrantDelegation = void 0;
...
class WarrantDelegation {
    ...
    async delegate(parentWarrant, request) {
        ...
        for (const action of request.delegated_scope) {
            if (!hasWildcard && !parentScope.has(action)) {
                throw new Error(`Scope violation: "${action}" is not in parent warrant scope. ` +
        ...
        if (requestedTtlMs > parentRemainingMs) {
            throw new Error(`TTL violation: requested ${request.ttl_minutes} minutes but parent only has ` +
        ...
        if (request.constraints) {
            this._validateConstraintStrictness(parentWarrant.constraints || {}, request.constraints);
        }
    ...
    async verify(warrantId) {
        ...
        const expectedSig = this._signDelegation(warrant.warrant_id, warrant.parent_warrant_id, warrant.root_warrant_id);
    ...
    async _walkChain(warrantId) {
        ...
        const now = Date.now();
        const remainingTtls = chain.map(w => Math.max(0, (new Date(w.expires_at).getTime() - now) / 1000));
    ...
    _validateConstraintStrictness(parentConstraints, childConstraints) {
        for (const [key, childC] of Object.entries(childConstraints)) {
            ...
            if (!parentC)
                continue; // New constraint is always fine (makes it stricter)
            // Child max must be ≤ parent max
            if (childC.max !== undefined && parentC.max !== undefined && childC.max > parentC.max) {
                throw new Error(`Constraint "${key}": child max (${childC.max}) cannot exceed parent max (${parentC.max})`);
            }
            // Child min must be ≥ parent min
            if (childC.min !== undefined && parentC.min !== undefined && childC.min < parentC.min) {
                throw new Error(`Constraint "${key}": child min (${childC.min}) cannot be below parent min (${parentC.min})`);
            }
            ...
            if (childC.enum && parentC.enum) {
                ...
                if (invalid.length > 0) {
                    throw new Error(`Constraint "${key}": child enum contains values not in parent: ${invalid.join(', ')}`);
                }
            }
        }
    }
...
exports.WarrantDelegation = WarrantDelegation;
...
exports.InMemoryDelegationStore = InMemoryDelegationStore;
exports.default = WarrantDelegation;
