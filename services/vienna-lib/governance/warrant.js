...
class Warrant {
     ...
     */
    async issue(options) {
        const { truthSnapshotId, planId, approvalId, approvalIds = [], objective, riskTier, allowedActions, forbiddenActions = [], constraints = {}, expiresInMinutes, rollbackPlan = null, justification = null, issuer = 'vienna' } = options;
    ...
    async listActive() {
        ...
        const now = new Date();
        return warrants.filter(w => w.status === 'issued' &&
    ...
    async _validateTruthFreshness(truth, riskTier) {
        ...
        if (ageMinutes > maxAge[riskTier]) {
            throw new Error(`Truth snapshot too old: ${ageMinutes.toFixed(1)} min (max ${maxAge[riskTier]} for ${riskTier})`);
        }
    }
    _generateChangeId() {
        const date = new Date();
    ...
    _assessTradingSafety(allowedActions) {
        ...
        const tradingInScope = allowedActions.some(action => tradingPatterns.some(pattern => action.toLowerCase().includes(pattern.toLowerCase())));
        if (!tradingInScope) {
