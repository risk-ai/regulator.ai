...
class ExpiryDetector {
    constructor(intervalMs = 15000) {
        ...
        this.running = false;
        this.staleWorkerThresholdMs = 60000; // 60 seconds
        this.abandonedClaimThresholdMs = 300000; // 5 minutes
    ...
    async detectExpiredLeases() {
        try {
            ...
            const abandonedClaims = await this.claimManager.findAbandonedClaims(this.abandonedClaimThresholdMs);
            ...
            for (const claim of abandonedClaims) {
                await this.claimManager.markAbandoned(claim.claim_id, `Abandoned after ${this.abandonedClaimThresholdMs}ms without completion`);
            ...
            for (const stuckItem of stuckItems) {
                try {
                    ...
                    console.log(`Recovered ${stuckItem.queue_item.id}: ${recoveryEvent.disposition} (${recoveryEvent.reason})`);
                }
                catch (error) {
...
exports.ExpiryDetector = ExpiryDetector;
