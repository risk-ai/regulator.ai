...
const worker_registry_1 = require("./worker-registry");
const lease_manager_1 = require("./lease-manager");
class QueueScheduler {
    constructor(intervalMs = 5000, workerId) {
        this.repository = new repository_1.QueueRepository();
        this.workerRegistry = new worker_registry_1.WorkerRegistry();
        this.leaseManager = new lease_manager_1.LeaseManager();
        this.claimManager = new ClaimManager();
        this.running = false;
        this.leaseTtlMs = 30000; // 30 seconds
    ...
    async runSchedulerCycle(deps) {
        ...
        try {
            ...
            for (const item of candidates) {
                const result = await (0, eligibility_1.isQueueItemEligible)(item, now, deps);
                if (result.eligible) {
                    eligibleItems.push(item);
                }
            }
            // Sort by priority
            eligibleItems.sort(eligibility_1.compareQueueItems);
    ...
    async processQueueItem(item, deps) {
        // Phase 16.4 Stage 1: Acquire exclusive lease (atomic CAS)
        const leaseResult = await this.leaseManager.acquireLease(item.id, this.workerId, this.leaseTtlMs);
        ...
        try {
            await (0, ledger_events_1.emitQueueLedgerEvent)("QUEUE_ITEM_LEASE_ACQUIRED", item, {
                lease_id: lease.lease_id,
                worker_id: this.workerId,
                expires_at: lease.expires_at,
            });
            // Phase 16.4 Stage 2: Acquire execution claim (atomic, exactly-once)
            const claimResult = await this.claimManager.acquireClaim(item.id, item.retry.attempt_count + 1, // Current attempt number
            this.workerId, { lease_id: lease.lease_id });
            ...
            await (0, ledger_events_1.emitQueueLedgerEvent)("QUEUE_ITEM_GOVERNANCE_REENTRY_STARTED", runningItem, {
            ...
            if (reentryResult.allowed) {
                ...
                await this.claimManager.markCompleted(claim.claim_id, this.workerId, `Execution ${reentryResult.execution_id} succeeded`);
                await (0, ledger_events_1.emitQueueLedgerEvent)("QUEUE_ITEM_GOVERNANCE_REENTRY_PASSED", runningItem, {
                ...
                await this.claimManager.markFailed(claim.claim_id, this.workerId, `Governance denied: ${reentryResult.transition_reason}`);
                // Governance denied - requeue or cancel
                if (reentryResult.disposition === "REQUEUE") {
                    await (0, ledger_events_1.emitQueueLedgerEvent)("QUEUE_ITEM_GOVERNANCE_REENTRY_REQUEUED", runningItem, {
                        transition_reason: reentryResult.transition_reason,
                    });
                    // Check if we should retry
                    if (item.retry_policy && (0, retry_1.shouldRetry)(item.retry, item.retry_policy)) {
                        const nextRetryAt = (0, retry_1.computeNextRetryAt)(new Date(), item.retry, item.retry_policy);
                        if (nextRetryAt) {
                            await this.repository.transitionItem({
                                queue_item_id: runningItem.id,
                                from_state: "RUNNING",
                    ...
                    // CANCEL
                    await (0, ledger_events_1.emitQueueLedgerEvent)("QUEUE_ITEM_GOVERNANCE_REENTRY_CANCELLED", runningItem, {
                        transition_reason: reentryResult.transition_reason,
                    });
                    await this.repository.transitionItem({
                        queue_item_id: runningItem.id,
            ...
            if (activeClaim) {
                await this.claimManager.markFailed(activeClaim.claim_id, this.workerId, errorMessage);
            }
            ...
            if (item.retry_policy && (0, retry_1.shouldRetry)(item.retry, item.retry_policy)) {
                const updatedRetry = (0, retry_1.incrementRetryAttempt)(item.retry, errorMetadata);
                const nextRetryAt = (0, retry_1.computeNextRetryAt)(new Date(), updatedRetry, item.retry_policy);
                if (nextRetryAt) {
                    await this.repository.transitionItem({
                        queue_item_id: item.id,
                        from_state: "RUNNING",
