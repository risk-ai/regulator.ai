...
export function createManagedObjectivesRouter(vienna) {
    const router = Router();
    /**
    ...
    router.get('/', async (req, res) => {
        try {
            const { status, target_type, objective_type, enabled, page = '1', pageSize = '50', } = req.query;
            // Parse pagination
            const pageNum = parseInt(page, 10);
            const limit = Math.min(parseInt(pageSize, 10), 200); // Max 200
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const stateGraph = getStateGraph();
            // Get objective
            const objectives = await stateGraph.query('SELECT * FROM managed_objectives WHERE objective_id = ?', [id]);
            ...
            const evaluations = await stateGraph.query('SELECT * FROM managed_objective_evaluations WHERE objective_id = ? ORDER BY started_at DESC LIMIT 1', [id]);
    ...
    router.get('/:id/evaluations', async (req, res) => {
        try {
            const { id } = req.params;
            const { since, limit = '100', status: statusFilter, } = req.query;
            const limitNum = Math.min(parseInt(limit, 10), 500); // Max 500
            // Get State Graph
            const { getStateGraph } = await import('../../../../lib/state/state-graph.js');
            const stateGraph = getStateGraph();
            // Build query
...
//# sourceMappingURL=managed-objectives.js.map