...
export class TimelineService {
    ...
    config;
    eventBuffer;
    constructor(viennaRuntime, config = {}) {
     ...
     */
    async getObjectiveTimeline(objectiveId, params) {
        try {
     ...
     */
    async getTimelineItems(objectiveId, params) {
        try {
    ...
    eventToTimelineItem(event) {
        ...
        if (event.payload) {
            if (event.payload.duration_ms)
                metadata.duration_ms = event.payload.duration_ms;
    ...
    cleanupBuffer() {
        ...
        for (const [objectiveId, events] of this.eventBuffer.events.entries()) {
            const filtered = events.filter(e => now - new Date(e.ts).getTime() < this.eventBuffer.retentionWindowMs);
