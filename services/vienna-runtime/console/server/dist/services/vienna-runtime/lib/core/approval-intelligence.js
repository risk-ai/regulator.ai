...
class ApprovalIntelligence {
    ...
    scoreRisk(approval) {
        ...
        // Risk tier base
        if (risk_tier === 'T0')
            baseScore = RiskScore.LOW;
        if (risk_tier === 'T1')
            baseScore = RiskScore.MEDIUM;
        if (risk_tier === 'T2')
            baseScore = RiskScore.HIGH;
        ...
        // Context modifiers
        if (context.trading_active)
            baseScore += 0.3;
        if (context.market_hours)
            baseScore += 0.2;
        if (context.production_env)
            baseScore += 0.15;
        if (context.high_concurrency)
            baseScore += 0.1;
        // Clamp to 0-1
    ...
    async groupApprovalsByRisk() {
        ...
        for (const approval of pending) {
            ...
            if (risk < RiskScore.MEDIUM)
                groups.low_risk.push(approval);
            else if (risk < RiskScore.HIGH)
                groups.medium_risk.push(approval);
            else if (risk < RiskScore.CRITICAL)
                groups.high_risk.push(approval);
            else
                groups.critical_risk.push(approval);
        }
    ...
    calculatePriority(approval, risk) {
        ...
        if (expiryMs < 60000)
            priority += 0.5; // < 1 minute: urgent
        else if (expiryMs < 300000)
            priority += 0.3; // < 5 min: soon
        else if (expiryMs < 600000)
            priority += 0.1; // < 10 min: moderate
        // Business impact
        if (approval.context?.trading_active)
            priority += 0.3;
        if (approval.context?.blocking_other_work)
            priority += 0.2;
    ...
    async setAutoExpiryPolicy(policy) {
        const { risk_threshold = RiskScore.MEDIUM, inactivity_ms = 3600000, // 1 hour default
        auto_deny = false } = policy;
        return {
        ...
        for (const approval of pending) {
            ...
            if (risk < policy.risk_threshold) {
                ...
                if (age > policy.inactivity_ms) {
                    ...
                    if (policy.auto_deny) {
                        await this.stateGraph.updateApprovalStatus(approval.approval_id, ApprovalState.DENIED, {
                            denied_by: 'system',
                        ...
                        await this.stateGraph.updateApprovalStatus(approval.approval_id, ApprovalState.EXPIRED, { policy_id: policyId });
                    }
                }
            }
        }
    ...
    async bulkApproveByPattern(pattern, reviewedBy) {
        ...
        for (const approval of matching) {
            await this.stateGraph.updateApprovalStatus(approval.approval_id, ApprovalState.APPROVED, {
...
//# sourceMappingURL=approval-intelligence.js.map