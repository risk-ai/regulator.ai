...
const { createVerificationTask, createWorkflowOutcome, deriveWorkflowStatus } = require('./verification-schema.js');
...
class ChatActionBridge {
    ...
    registerAction(action) {
        const { action_id, action_name, risk_tier, target_endpoint, handler } = action;
            ...
            if (plan && plan.verification_spec && this.stateGraph) {
                ...
                if (result.success) {
                    try {
                        ...
                        const verificationTask = this._buildVerificationTask(plan, result, executionContext);
                ...
                const summary = this._generateWorkflowSummary(plan.objective, executionStatus, verificationStatus, objectiveAchieved);
    ...
    async executePlan(planId, context = {}) {
        ...
        if (this.policyEngine) {
            policyDecision = await this.policyEngine.evaluatePolicies(plan.objective, plan.steps[0].action, plan.risk_tier, context);
        ...
        if (plan.verification_spec) {
            ...
            workflowOutcome = createWorkflowOutcome({
                ...
                workflow_status: workflowStatus,
                objective_achieved: verificationResult.objective_achieved,
                summary: this._generateWorkflowSummary(plan.objective, executionStatus, verificationStatus, verificationResult.objective_achieved),
                metadata: {
...
//# sourceMappingURL=chat-action-bridge.js.map