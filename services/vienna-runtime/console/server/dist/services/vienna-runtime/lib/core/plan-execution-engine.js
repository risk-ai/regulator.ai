...
const { ResolutionOutcome, resolveApprovalStatus, validateApprovalForResumption, getLedgerEventType } = require('./approval-resolution-handler');
/**
...
class PlanExecutionEngine {
    ...
    _determinePlanOutcome(execContext) {
        ...
        const allCompleted = states.every(s => s.status === StepStatus.COMPLETED || s.status === StepStatus.SKIPPED);
        if (hasFailed)
            return 'failed';
        if (hasBlocked)
            return 'blocked';
    ...
    async _checkApprovalResolution(step, execContext, context) {
        ...
        const approval = await this.approvalManager.getApprovalByContext(context.execution_id, step.step_id);
...
//# sourceMappingURL=plan-execution-engine.js.map