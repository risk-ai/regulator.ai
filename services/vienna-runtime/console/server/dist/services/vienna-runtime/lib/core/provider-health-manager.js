...
class ProviderHealthManager {
    ...
    async recordFailure(name, error) {
        ...
        state.lastHealthCheck = now; // Update health check timestamp
        console.warn(`[ProviderHealthManager] Provider ${name} failure ` +
    ...
    async quarantineProvider(name) {
        ...
        console.error(`[ProviderHealthManager] Provider ${name} QUARANTINED until ${quarantineUntil} ` +
    ...
    hasQuarantineExpired(name) {
        const state = this.providers.get(name);
        if (!state || !state.quarantineUntil)
            return true;
    ...
    getRuntimeStatus(providers) {
        ...
        if (quarantined > 0 || degraded > 0 || unhealthy > 0)
            return 'degraded';
        // All healthy = operational
        if (healthy === providers.length)
            return 'operational';
        return 'degraded';
    }
}
module.exports = { ProviderHealthManager };
//# sourceMappingURL=provider-health-manager.js.map