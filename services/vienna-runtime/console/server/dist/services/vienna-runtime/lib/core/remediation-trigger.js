...
async function triggerRemediation(objectiveId, context = {}) {
    ...
    stateGraph.updateObjectiveStatus(objectiveId, 'remediation_triggered', 'evaluation', // transition_reason
    {
    ...
    if (!remediationPlan) {
        ...
        stateGraph.updateObjectiveStatus(objectiveId, 'failed', 'execution', {
    ...
    try {
        ...
        stateGraph.updateObjectiveStatus(objectiveId, 'remediation_running', 'execution', {
            plan_id: remediationPlan.plan_id,
            started_at: new Date().toISOString()
        });
        // Execute through governed pipeline (Phase 9.7.3: Real Execution)
        const executionResult = await context.chatActionBridge.executeRemediationPlan(remediationPlan.plan_id, { objectiveId });
        // Extract results
        ...
        if (executionResult.status === 'failed') {
            stateGraph.updateObjectiveStatus(objectiveId, 'failed', 'execution', { ...transitionMetadata, execution_error: executionResult.error });
            return {
                objective_id: objectiveId,
                objective_state: 'failed',
                triggered_plan_id: remediationPlan.plan_id,
        ...
        stateGraph.updateObjectiveStatus(objectiveId, 'verification', 'execution', transitionMetadata);
        ...
        stateGraph.updateObjectiveStatus(objectiveId, finalState, finalReason, finalMetadata);
        return {
            objective_id: objectiveId,
...
//# sourceMappingURL=remediation-trigger.js.map