...
class StructuredLogger {
    ...
    _logToConsole(entry) {
        ...
        if (entry.envelope_id)
            parts.push(`[env:${entry.envelope_id}]`);
        if (entry.objective_id)
            parts.push(`[obj:${entry.objective_id}]`);
        if (entry.provider)
    ...
    async query(criteria = {}) {
        ...
        return recent.filter(log => {
            ...
            if (criteria.level && log.level !== criteria.level)
                return false;
            if (criteria.envelope_id && log.envelope_id !== criteria.envelope_id)
                return false;
            if (criteria.objective_id && log.objective_id !== criteria.objective_id)
                return false;
            if (criteria.provider && log.provider !== criteria.provider)
                return false;
            return true;
        });
    }
