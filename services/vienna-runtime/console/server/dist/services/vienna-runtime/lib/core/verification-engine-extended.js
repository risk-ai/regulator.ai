...
const { EXTENDED_VERIFICATION_TEMPLATES, FailureClass, classifyFailure, shouldRetry, getBackoffDelay } = require('./verification-templates-extended');
/**
...
class ExtendedVerificationEngine extends VerificationEngine {
    ...
    validateTemplateBinding(verificationTask) {
        ...
        if (template.required_strength &&
            ...
            errors.push(`Verification strength mismatch: expected ${template.required_strength}, got ${verificationTask.required_strength}`);
        }
        // Timeout check
        if (template.timeout_ms && verificationTask.timeout_ms < template.timeout_ms) {
            errors.push(`Timeout too short: expected at least ${template.timeout_ms}ms, got ${verificationTask.timeout_ms}ms`);
        }
        ...
        if (missingChecks.length > 0) {
            errors.push(`Missing required checks: ${missingChecks.join(', ')}`);
        }
        return {
...
//# sourceMappingURL=verification-engine-extended.js.map