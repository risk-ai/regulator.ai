...
class ExecutionCoordinator {
    ...
    async dispatchExecution(executionId, plan, context, options = {}) {
        ...
        const distributionId = await this.workDistributor.createDistribution(executionId, plan.plan_id, selection.node, selection.candidates, selection.strategy);
        // Acquire lock
        ...
        const coordinationId = await this._createCoordination(executionId, selection.node.node_id, plan, context, lockId);
        // Send execution request to node
        const result = await this._sendExecuteRequest(selection.node, executionId, plan, context, lockId);
        // Update status
    ...
    async getCoordination(coordinationId) {
        const row = await this.stateGraph.get(`SELECT * FROM execution_coordinations WHERE coordination_id = ?`, [coordinationId]);
        if (!row)
            return null;
        return {
    ...
    async _createCoordination(executionId, nodeId, plan, context, lockId) {
        ...
        await this.stateGraph.run(`INSERT INTO execution_coordinations (
    ...
    async _updateCoordination(coordinationId, updates) {
        ...
        await this.stateGraph.run(`UPDATE execution_coordinations SET ${fields.join(', ')} WHERE coordination_id = ?`, params);
    }
