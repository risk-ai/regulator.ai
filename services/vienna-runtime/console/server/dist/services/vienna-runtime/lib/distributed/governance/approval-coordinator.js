...
class ApprovalCoordinator {
    ...
    async requestApproval(executionId, requestingNodeId, plan, context) {
        ...
        await this.stateGraph.run(`INSERT INTO cross_node_approvals (
    ...
    async getApproval(approvalId) {
        const row = await this.stateGraph.get(`SELECT * FROM cross_node_approvals WHERE approval_id = ?`, [approvalId]);
        if (!row)
            return null;
        return {
    ...
    async listPendingApprovals() {
        const rows = await this.stateGraph.all(`SELECT * FROM cross_node_approvals 
    ...
    async _updateApproval(approvalId, updates) {
        ...
        await this.stateGraph.run(`UPDATE cross_node_approvals 
       SET ${fields.join(', ')} 
