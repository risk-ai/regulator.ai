...
class FederatedLedger {
    ...
    async emitEvent(sourceNodeId, eventType, executionId, metadata = {}) {
        ...
        await this.stateGraph.run(`INSERT INTO federated_ledger_events (
    ...
    async getExecutionEvents(executionId) {
        const rows = await this.stateGraph.all(`SELECT * FROM federated_ledger_events 
       WHERE execution_id = ? 
       ORDER BY timestamp ASC`, [executionId]);
...
//# sourceMappingURL=federated-ledger.js.map