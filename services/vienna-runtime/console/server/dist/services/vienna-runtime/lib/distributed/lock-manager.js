...
class LockManager {
    ...
    async acquireLock(lockRequest) {
        ...
        const existing = await this.stateGraph.get(`SELECT * FROM distributed_locks 
       WHERE target_type = ? AND target_id = ? AND status = 'active'`, [target_type, target_id]);
        if (existing) {
        ...
        await this.stateGraph.run(`INSERT INTO distributed_locks (
    ...
    async releaseLock(lockId) {
        await this.stateGraph.run(`UPDATE distributed_locks SET status = 'released' WHERE lock_id = ?`, [lockId]);
    }
    /**
     * Get lock
     */
    async getLock(lockId) {
        const row = await this.stateGraph.get(`SELECT * FROM distributed_locks WHERE lock_id = ?`, [lockId]);
        if (!row)
            return null;
        return {
            lock_id: row.lock_id,
    ...
    async cleanupExpiredLocks() {
        ...
        const expired = await this.stateGraph.all(`SELECT * FROM distributed_locks 
       WHERE status = 'active' AND expires_at < ?`, [now]);
        for (const lock of expired) {
            await this.stateGraph.run(`UPDATE distributed_locks SET status = 'expired' WHERE lock_id = ?`, [lock.lock_id]);
        }
    ...
    async listLocksByNode(nodeId) {
        const rows = await this.stateGraph.all(`SELECT * FROM distributed_locks 
       WHERE locked_by_node_id = ? AND status = 'active'`, [nodeId]);
        return rows.map(r => ({
            lock_id: r.lock_id,
    ...
    async releaseAllLocksForNode(nodeId) {
        await this.stateGraph.run(`UPDATE distributed_locks 
       SET status = 'released' 
       WHERE locked_by_node_id = ? AND status = 'active'`, [nodeId]);
    }
    _generateId(prefix) {
