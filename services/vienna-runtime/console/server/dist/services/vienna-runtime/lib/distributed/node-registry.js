...
class NodeRegistry {
    ...
    async _registerNodeAsync(node) {
        ...
        await this.stateGraph.run(`INSERT INTO execution_nodes (
    ...
    async getNode(nodeId) {
        const row = await this.stateGraph.get(`SELECT * FROM execution_nodes WHERE node_id = ?`, [nodeId]);
        if (!row)
            return null;
    ...
    async updateHeartbeat(nodeId) {
        const now = new Date().toISOString();
        await this.stateGraph.run(`UPDATE execution_nodes 
       SET last_heartbeat_at = ?, status = 'online' 
       WHERE node_id = ?`, [now, nodeId]);
    ...
    async updateCapabilities(nodeId, capabilities) {
        await this.stateGraph.run(`UPDATE execution_nodes 
       SET capabilities = ? 
       WHERE node_id = ?`, [JSON.stringify(capabilities), nodeId]);
    }
    /**
     * Update node status
     */
    ...
    async checkStaleNodes() {
        ...
        const staleNodes = await this.stateGraph.all(`SELECT * FROM execution_nodes 
    ...
    async findCapableNodes(plan) {
        ...
        return allNodes.filter(node => {
            return requiredCapabilities.every(req => {
                return node.capabilities.some(cap => cap.action_type === req.action_type &&
    ...
    async deregisterNode(nodeId) {
        await this.stateGraph.run(`DELETE FROM execution_nodes WHERE node_id = ?`, [nodeId]);
    }
    // Helper methods
