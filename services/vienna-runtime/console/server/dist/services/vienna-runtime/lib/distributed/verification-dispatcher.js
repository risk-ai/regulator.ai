...
class VerificationDispatcher {
    ...
    async dispatchVerification(executionId, verificationTask, options = {}) {
        ...
        await this.stateGraph.run(`INSERT INTO remote_verifications (
        ...
        const result = await this._sendVerificationRequest(selectedNode, verificationId, verificationTask);
        // Update status
    ...
    async handleVerificationResult(verificationId, result) {
        await this.stateGraph.run(`UPDATE remote_verifications 
    ...
    async getVerification(verificationId) {
        const row = await this.stateGraph.get(`SELECT * FROM remote_verifications WHERE verification_id = ?`, [verificationId]);
        if (!row)
            return null;
        return {
    ...
    async updateVerificationStatus(verificationId, status) {
        await this.stateGraph.run(`UPDATE remote_verifications SET status = ? WHERE verification_id = ?`, [status, verificationId]);
    }
    // Helper methods
...
//# sourceMappingURL=verification-dispatcher.js.map