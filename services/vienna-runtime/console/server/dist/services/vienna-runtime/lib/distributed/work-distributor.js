...
class WorkDistributor {
    ...
    async createDistribution(executionId, planId, selectedNode, candidateNodes, strategy) {
        ...
        await this.stateGraph.run(`INSERT INTO work_distributions (
    ...
    async updateDistributionStatus(distributionId, status) {
        await this.stateGraph.run(`UPDATE work_distributions SET status = ? WHERE distribution_id = ?`, [status, distributionId]);
    }
    /**
     * Get distribution
     */
    async getDistribution(distributionId) {
        const row = await this.stateGraph.get(`SELECT * FROM work_distributions WHERE distribution_id = ?`, [distributionId]);
        if (!row)
            return null;
        return {
...
//# sourceMappingURL=work-distributor.js.map