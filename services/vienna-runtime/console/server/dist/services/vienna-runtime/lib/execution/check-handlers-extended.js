...
async function checkDatabaseQuery(parameters, context = {}) {
    ...
    try {
        ...
        if (contains && Array.isArray(contains)) {
            ...
            const allFound = contains.every(term => output.includes(term.toLowerCase()));
            if (!allFound) {
                return {
                    success: false,
...
async function checkSystemdLog(parameters, context = {}) {
    ...
    try {
        ...
        const foundPatterns = not_contains.filter(pattern => logs.includes(pattern.toLowerCase()));
...
//# sourceMappingURL=check-handlers-extended.js.map