...
class ExecutionQueue {
    ...
    async enqueue(envelope) {
        ...
        if (!backpressureCheck.allowed) {
            if (this.backpressureMode === 'reject') {
                throw new BackpressureError(backpressureCheck.reason, backpressureCheck.queueSize, backpressureCheck.maxSize);
            }
    ...
    async _waitForQueueSpace() {
        ...
        // Timeout reached
        throw new BackpressureError(`Queue backpressure timeout: waited ${this.backpressureWaitTimeout}ms`, this.getStats().queued + this.getStats().executing, this.maxQueueSize);
    }
    /**
