...
class Executor {
        ...
        try {
            ...
            for (const action of envelope.actions) {
                ...
                if (!actionResult.success && envelope.fail_fast !== false) {
                    ...
                    throw new ExecutionError('ACTION_FAILED', `Action ${action.type} failed: ${actionResult.error}`);
                }
            }
    ...
    _validateEnvelope(envelope) {
        ...
        if (missing.length > 0) {
            throw new ExecutionError('INVALID_ENVELOPE', `Missing required fields: ${missing.join(', ')}`);
        }
        if (!Array.isArray(envelope.actions) || envelope.actions.length === 0) {
            throw new ExecutionError('INVALID_ENVELOPE', 'Envelope must have at least one action');
        }
        // Validate each action
        for (const action of envelope.actions) {
            if (!action.type || !action.target) {
                throw new ExecutionError('INVALID_ACTION', 'Each action must have type and target');
            }
        }
    }
    /**
    ...
    async _verifyWarrant(warrantId) {
        ...
        if (!verification.valid) {
            throw new ExecutionError('WARRANT_INVALID', `Warrant verification failed: ${verification.reason}`);
        }
    ...
    async _runPreflightChecks(envelope, warrant) {
        ...
        const tradingCriticalActions = envelope.actions.filter(a => this._isTradingCritical(a));
        if (tradingCriticalActions.length > 0) {
            const guardResult = await this.viennaCore.tradingGuard.check(tradingCriticalActions);
            if (!guardResult.safe) {
                throw new ExecutionError('TRADING_GUARD_BLOCKED', guardResult.message || guardResult.reason);
            }
        }
        ...
        for (const action of envelope.actions) {
            ...
            if (!warrant.allowed_actions.includes(actionStr)) {
                throw new ExecutionError('ACTION_NOT_IN_SCOPE', `Action ${actionStr} not allowed by warrant`);
            }
        }
        ...
        if (now > expires) {
            throw new ExecutionError('WARRANT_EXPIRED', `Warrant expired at ${warrant.expires_at}`);
        }
    }
    /**
    ...
    _isTradingCritical(action) {
        ...
        const targetMatches = tradingPatterns.some(pattern => action.target?.toLowerCase().includes(pattern.toLowerCase()));
        const typeMatches = criticalTypes.some(critical => action.type?.startsWith(critical));
