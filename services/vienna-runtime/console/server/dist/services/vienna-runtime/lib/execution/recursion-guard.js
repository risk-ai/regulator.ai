...
class RecursionGuard {
    ...
    validate(envelope) {
        ...
        const duplicateCheck = this._checkIdempotency(envelope.idempotency_key, policy.duplicate_window_seconds);
        ...
        const cooldownCheck = this._checkCooldown(envelope, policy.cooldown_seconds);
    ...
    recordExecution(envelope) {
        ...
        if (envelope.attempt === 0) {
            ...
            if (currentBudget === undefined) {
                ...
                this.triggerBudgets.set(envelope.trigger_id, policy.max_descendants_per_root - 1);
            }
            else {
    ...
    cleanup() {
        const now = Date.now();
        const maxAge = Math.max(this.policy.duplicate_window_seconds, this.policy.cooldown_seconds) * 1000;
