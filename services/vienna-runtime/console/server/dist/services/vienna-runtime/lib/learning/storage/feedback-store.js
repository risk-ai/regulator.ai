...
class FeedbackStore {
    ...
    async recordFeedback(feedback) {
        ...
        await this.stateGraph.run(`INSERT INTO operator_feedback (
    ...
    async markFeedbackProcessed(feedbackId, metadata = {}) {
        await this.stateGraph.run(`UPDATE operator_feedback 
       SET processed = 1, processed_at = ? 
       WHERE feedback_id = ?`, [new Date().toISOString(), feedbackId]);
    }
    /**
    ...
    async archiveFeedback(feedbackId) {
        const row = await this.stateGraph.get(`SELECT * FROM operator_feedback WHERE feedback_id = ?`, [feedbackId]);
        if (!row) {
            throw new Error(`Feedback not found: ${feedbackId}`);
        }
        await this.stateGraph.run(`DELETE FROM operator_feedback WHERE feedback_id = ?`, [feedbackId]);
