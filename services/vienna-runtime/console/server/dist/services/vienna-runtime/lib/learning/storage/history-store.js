...
class HistoryStore {
    ...
    async recordHistory(history) {
        ...
        await this.stateGraph.run(`INSERT INTO learning_history (
    ...
    async getRecommendationHistory(recommendationId) {
        const rows = await this.stateGraph.all(`SELECT * FROM learning_history 
       WHERE recommendation_id = ? 
       ORDER BY timestamp ASC`, [recommendationId]);
    ...
    async getLearningImpact(filters = {}) {
       ...
       AND action = 'applied'`, recommendations);
        if (rows.length === 0) {
            return null;
        }
    ...
    async archiveHistory(historyId) {
        const row = await this.stateGraph.get(`SELECT * FROM learning_history WHERE history_id = ?`, [historyId]);
        if (!row) {
            throw new Error(`History not found: ${historyId}`);
        }
        await this.stateGraph.run(`DELETE FROM learning_history WHERE history_id = ?`, [historyId]);
