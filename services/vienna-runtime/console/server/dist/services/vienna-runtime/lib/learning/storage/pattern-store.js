...
class PatternStore {
    ...
    async createPattern(pattern) {
        ...
        await this.stateGraph.run(`INSERT INTO learning_patterns (
    ...
    async getPattern(patternId) {
        const row = await this.stateGraph.get(`SELECT * FROM learning_patterns WHERE pattern_id = ?`, [patternId]);
        if (!row)
            return null;
    ...
    async updatePattern(patternId, updates) {
        ...
        await this.stateGraph.run(`UPDATE learning_patterns SET ${fields.join(', ')} WHERE pattern_id = ?`, params);
    }
    /**
    ...
    async getPatternEvolution(patternId) {
        ...
        while (currentId) {
            ...
            const next = await this.stateGraph.get(`SELECT * FROM learning_patterns WHERE superseded_by = ?`, [currentId]);
    ...
    async archivePattern(patternId) {
        ...
        await this.stateGraph.run(`DELETE FROM learning_patterns WHERE pattern_id = ?`, [patternId]);
        return pattern;
    }
    // Helper methods
