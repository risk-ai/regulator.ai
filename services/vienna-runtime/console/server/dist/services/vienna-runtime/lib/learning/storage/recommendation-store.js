...
class RecommendationStore {
    ...
    async createRecommendation(recommendation) {
        ...
        await this.stateGraph.run(`INSERT INTO learning_recommendations (
    ...
    async getRecommendation(recommendationId) {
        const row = await this.stateGraph.get(`SELECT * FROM learning_recommendations WHERE recommendation_id = ?`, [recommendationId]);
        if (!row)
            return null;
    ...
    async updateRecommendationStatus(recommendationId, status, metadata = {}) {
        ...
        await this.stateGraph.run(`UPDATE learning_recommendations SET ${fields.join(', ')} WHERE recommendation_id = ?`, params);
    }
    /**
    ...
    async archiveRecommendation(recommendationId) {
        ...
        await this.stateGraph.run(`DELETE FROM learning_recommendations WHERE recommendation_id = ?`, [recommendationId]);
