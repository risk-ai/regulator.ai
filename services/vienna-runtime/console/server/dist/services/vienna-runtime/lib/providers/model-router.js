...
class ModelRouter {
     ...
     */
    route(request) {
        const { task_type = 'general', operator, required_capabilities = [], max_cost_class, } = request;
        ...
        if (allRequiredCapabilities.length > 0) {
            candidates = candidates.filter(m => allRequiredCapabilities.every(cap => m.capabilities.includes(cap)));
        }
