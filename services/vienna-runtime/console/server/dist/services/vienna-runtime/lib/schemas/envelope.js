...
function validateEnvelope(envelope) {
    ...
    if (!envelope.envelope_id)
        errors.push('Missing envelope_id');
    if (!envelope.warrant_id)
        errors.push('Missing warrant_id');
    ...
    if (!envelope.objective_id)
        errors.push('Missing objective_id');
    if (!envelope.trigger_id)
        errors.push('Missing trigger_id');
    if (!envelope.origin_type)
        errors.push('Missing origin_type');
    if (!envelope.origin_id)
        errors.push('Missing origin_id');
    if (!envelope.trigger_type)
    ...
    if (!envelope.idempotency_key)
        errors.push('Missing idempotency_key');
    // Validate actions
    if (Array.isArray(envelope.actions)) {
        envelope.actions.forEach((action, idx) => {
            if (!action.type)
                errors.push(`Action ${idx}: missing type`);
            if (!action.target)
                errors.push(`Action ${idx}: missing target`);
        });
    }
    return {
...
function createEnvelope(params) {
    const { actions, warrant_id, objective_id = generateObjectiveId(), trigger_id = generateTriggerId(), parent_envelope_id = null, causal_depth = 0, attempt = 0, loop_budget_remaining = 5, // Default from Phase 7.3 directive
    origin_type = 'agent', origin_id, trigger_type = 'operator_directive', proposed_by, fail_fast = true, rollback_plan = null, preconditions = [] } = params;
...
function createDescendantEnvelope(parentEnvelope, params) {
    ...
    const { actions, warrant_id, origin_id, proposed_by } = params;
