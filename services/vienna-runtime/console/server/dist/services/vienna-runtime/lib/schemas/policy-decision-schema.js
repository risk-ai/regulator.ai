 ...
 */
function createPolicyDecision({ plan_id, policy_id, policy_version, decision, reasons, requirements, evaluated_context, conflict_resolution }) {
    ...
    for (const reqs of requirementsArray) {
        ...
        if (reqs.allowed_actor_types && reqs.allowed_actor_types.length > 0) {
            if (merged.allowed_actor_types.length === 0) {
                ...
                merged.allowed_actor_types = merged.allowed_actor_types.filter(a => reqs.allowed_actor_types.includes(a));
            }
        }
        ...
        if (reqs.min_time_between_executions_minutes) {
            ...
            merged.min_time_between_executions_minutes = Math.max(current, reqs.min_time_between_executions_minutes);
        }
    }
    return merged;
}
...
//# sourceMappingURL=policy-decision-schema.js.map