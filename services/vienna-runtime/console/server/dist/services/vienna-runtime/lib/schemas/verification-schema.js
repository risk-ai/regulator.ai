 ...
 */
function createVerificationTask({ plan_id, execution_id, objective, verification_type, scope, postconditions, verification_strength = VerificationStrength.SERVICE_HEALTH, timeout_ms = 15000, stability_window_ms = 0, retry_policy = { max_attempts: 3, backoff_ms: 1000 }, created_by = 'vienna-core' }) {
    return {
 ...
 */
function createVerificationResult({ verification_id, plan_id, execution_id, status, objective_achieved, verification_strength_achieved, started_at, completed_at, checks, stability = null, summary, metadata = {} }) {
    return {
        verification_id,
        plan_id,
        execution_id,
 ...
 */
function createWorkflowOutcome({ plan_id, execution_id = null, verification_id = null, workflow_status, objective_achieved, risk_tier, execution_status = null, verification_status = null, operator_visible_summary, next_actions = [], metadata = {} }) {
    return {
...
function validateVerificationTask(task) {
    ...
    if (!task.verification_id)
        errors.push('verification_id required');
    if (!task.plan_id)
        errors.push('plan_id required');
    if (!task.objective)
        errors.push('objective required');
    if (!task.verification_type)
        errors.push('verification_type required');
    if (!Array.isArray(task.postconditions))
        errors.push('postconditions must be array');
