...
function getDefaultDbPath() {
    ...
    return path.join(process.env.HOME, '.openclaw', 'runtime', env, 'state', 'state-graph.db');
}
class StateGraph {
    constructor(options = {}) {
    ...
    `);
        const info = stmt.run(service.service_id, service.service_name, service.service_type, service.status, service.health || null, service.last_check_at || null, service.last_restart_at || null, service.dependencies ? JSON.stringify(service.dependencies) : null, service.metadata ? JSON.stringify(service.metadata) : null, now, now);
    ...
    `);
        const info = stmt.run(provider.provider_id, provider.provider_name, provider.provider_type, provider.status, provider.health || null, provider.last_health_check || null, provider.credentials_status || null, provider.rate_limit_info ? JSON.stringify(provider.rate_limit_info) : null, provider.error_count || 0, provider.last_error_at || null, provider.metadata ? JSON.stringify(provider.metadata) : null, now, now);
    ...
    `);
        const info = stmt.run(incident.incident_id, incident.incident_type, incident.severity, incident.status, incident.affected_services ? JSON.stringify(incident.affected_services) : null, incident.detected_at, incident.detected_by || null, incident.resolved_at || null, incident.resolution || null, incident.root_cause || null, incident.action_taken || null, incident.pattern_id || null, incident.metadata ? JSON.stringify(incident.metadata) : null, now, now);
    ...
    `);
        const info = stmt.run(objective.objective_id, objective.objective_name, objective.objective_type, objective.status, objective.priority || null, objective.assigned_to || null, objective.blocked_reason || null, objective.dependencies ? JSON.stringify(objective.dependencies) : null, objective.completion_criteria || null, objective.progress_pct || 0, objective.started_at || null, objective.completed_at || null, objective.due_at || null, objective.metadata ? JSON.stringify(objective.metadata) : null, now, now);
    ...
    setRuntimeContext(contextKey, contextValue, options = {}) {
        ...
        WHERE context_key = ?
      `);
            const info = stmt.run(contextValue, options.context_type || null, options.expires_at || null, options.metadata ? JSON.stringify(options.metadata) : null, now, contextKey);
      ...
      `);
            const info = stmt.run(contextKey, contextValue, options.context_type || null, options.expires_at || null, options.metadata ? JSON.stringify(options.metadata) : null, now, now);
            return { context_key: contextKey, changes: info.changes };
        }
    }
    ...
    `);
        const info = stmt.run(endpoint.endpoint_id, endpoint.endpoint_type, endpoint.endpoint_name, endpoint.status, endpoint.health, endpoint.connectivity || null, endpoint.last_heartbeat || null, endpoint.last_successful_action || null, endpoint.capabilities || null, endpoint.version || null, endpoint.metadata || null, now, now);
    ...
    `);
        const info = stmt.run(instruction.instruction_id, instruction.endpoint_id, instruction.instruction_type, instruction.action, instruction.risk_tier, instruction.warrant_id || null, instruction.issued_by, instruction.issued_at, instruction.status);
    ...
    `);
        const info = stmt.run(plan.plan_id, plan.objective, plan.intent_id || null, JSON.stringify(plan.steps), plan.preconditions ? JSON.stringify(plan.preconditions) : null, plan.postconditions ? JSON.stringify(plan.postconditions) : null, plan.risk_tier, plan.estimated_duration_ms || null, plan.status, plan.verification_spec ? JSON.stringify(plan.verification_spec) : null, plan.warrant_id || null, plan.execution_id || null, plan.result ? JSON.stringify(plan.result) : null, plan.error || null, plan.actual_duration_ms || null, plan.metadata ? JSON.stringify(plan.metadata) : null, now, now);
    ...
    `);
        const info = stmt.run(verification.verification_id, verification.plan_id, verification.execution_id || null, verification.verification_type, verification.status, verification.objective_achieved ? 1 : 0, verification.verification_strength_target || null, verification.verification_strength_achieved || null, verification.started_at, verification.completed_at || null, verification.duration_ms || null, verification.summary || null, verification.evidence_json ? JSON.stringify(verification.evidence_json) : null, verification.metadata ? JSON.stringify(verification.metadata) : null, now);
    ...
    `);
        const info = stmt.run(outcome.outcome_id, outcome.plan_id, outcome.execution_id || null, outcome.verification_id || null, outcome.workflow_status, outcome.execution_status || null, outcome.verification_status || null, outcome.objective_achieved ? 1 : 0, outcome.risk_tier, outcome.finalized_at, outcome.operator_visible_summary || null, outcome.next_actions ? JSON.stringify(outcome.next_actions) : null, outcome.metadata ? JSON.stringify(outcome.metadata) : null, now);
    ...
    appendLedgerEvent(event) {
        ...
        if (!event.sequence_num) {
            const maxSeq = this.db.prepare('SELECT COALESCE(MAX(sequence_num), 0) as max_seq FROM execution_ledger_events WHERE execution_id = ?').get(event.execution_id);
    ...
    `);
        const info = stmt.run(event.event_id, event.execution_id, event.plan_id || null, event.verification_id || null, event.warrant_id || null, event.outcome_id || null, event.event_type, event.stage, event.actor_type || null, event.actor_id || null, event.environment || this.environment, event.risk_tier || null, event.objective || null, event.target_type || null, event.target_id || null, event.event_timestamp, event.sequence_num, event.status || null, event.payload_json ? JSON.stringify(event.payload_json) : null, event.evidence_json ? JSON.stringify(event.evidence_json) : null, event.summary || null, now);
    ...
    _projectEventIntoSummary(event) {
        ...
        if (!summary) {
            ...
            if (event.plan_id && !summary.plan_id)
                summary.plan_id = event.plan_id;
            if (event.verification_id && !summary.verification_id)
                summary.verification_id = event.verification_id;
            if (event.warrant_id && !summary.warrant_id)
                summary.warrant_id = event.warrant_id;
            if (event.outcome_id && !summary.outcome_id)
                summary.outcome_id = event.outcome_id;
        }
        ...
        switch (event.event_type) {
            ...
            case 'plan_created':
                ...
                if (payload.target_type)
                    summary.target_type = payload.target_type;
                if (payload.target_id)
                    summary.target_id = payload.target_id;
                if (payload.risk_tier)
                    summary.risk_tier = payload.risk_tier;
                break;
            ...
            case 'workflow_outcome_finalized':
                ...
                if (payload.final_summary)
                    summary.summary = payload.final_summary;
                summary.completed_at = event.event_timestamp;
    ...
    `);
        upsert.run(summary.execution_id, summary.plan_id, summary.verification_id, summary.warrant_id, summary.outcome_id, summary.actor_type, summary.actor_id, summary.environment, summary.risk_tier, summary.objective, summary.target_type, summary.target_id, summary.current_stage, summary.execution_status, summary.verification_status, summary.workflow_status, summary.objective_achieved, summary.approval_required, summary.approval_status, summary.started_at, summary.completed_at, summary.duration_ms, summary.event_count, summary.last_event_type, summary.last_event_timestamp, summary.summary, summary.entities_json, summary.created_at || now, now);
    }
    /**
    ...
    savePolicy(policy) {
      ...
      ON CONFLICT(policy_id, policy_version) DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `);
        stmt.run(policy.policy_id, policy.policy_version, JSON.stringify(policy), policy.enabled ? 1 : 0, policy.priority, policy.description || null, now, now);
        return policy;
    }
    /**
    ...
    `);
        stmt.run(decision.decision_id, decision.plan_id, decision.policy_id, decision.policy_version, decision.decision, JSON.stringify(decision), decision.timestamp, now);
        return decision;
    }
    /**
    ...
    `);
        stmt.run(objective.objective_id, objective.objective_type || 'custom', objective.target_type || 'service', objective.target_id, this.environment, objective.status, JSON.stringify(objective.desired_state), objective.remediation_plan, objective.evaluation_interval_seconds || this._parseInterval(objective.evaluation_interval), objective.verification_strength, objective.priority || 100, objective.owner || 'system', JSON.stringify(objective.context || {}), objective.reconciliation_status || 'idle', objective.reconciliation_attempt_count || 0, objective.reconciliation_started_at || null, objective.reconciliation_cooldown_until || null, objective.reconciliation_last_result || null, objective.reconciliation_last_error || null, objective.reconciliation_last_execution_id || null, objective.reconciliation_last_verified_at || null, objective.reconciliation_generation || 0, objective.manual_hold !== undefined ? (objective.manual_hold ? 1 : 0) : 0, objective.created_at || now, objective.updated_at || now, objective.is_enabled !== undefined ? (objective.is_enabled ? 1 : 0) : 1);
    ...
    updateObjectiveStatus(objectiveId, newStatus, reason, metadata = {}) {
        ...
        if (!isValidTransition(objective.status, newStatus)) {
            ...
            throw new Error(`Invalid transition: ${objective.status} → ${newStatus}. ` +
    ...
    `);
        stmt.run(evaluationId, evaluation.objective_id, evaluation.evaluation_timestamp || now, JSON.stringify(evaluation.observed_state), evaluation.objective_satisfied ? 1 : 0, evaluation.violation_detected ? 1 : 0, evaluation.action_taken || 'none', evaluation.result_summary || null, evaluation.triggered_plan_id || null, evaluation.triggered_execution_id || null);
    ...
    `);
        stmt.run(historyId, objectiveId, fromStatus || null, toStatus, reason || null, JSON.stringify(metadata), now);
        return historyId;
    }
    /**
    ...
    `);
        stmt.run(policy.policy_id, policy.policy_name, policy.description || null, policy.max_consecutive_failures || null, policy.cooldown?.mode || null, policy.cooldown?.base_seconds || null, policy.cooldown?.multiplier || null, policy.cooldown?.max_seconds || null, policy.degraded?.enter_after_consecutive_failures || null, policy.reset?.on_verified_recovery !== false ? 1 : 0, policy.reset?.on_manual_reset !== false ? 1 : 0, policy.created_at || now, policy.updated_at || now);
    ...
    `);
        stmt.run(intent_id, intent_type, JSON.stringify(source), submitted_at || now, now);
        return intent_id;
    }
    /**
    ...
    `);
        stmt.run(anomalyData.anomaly_id, anomalyData.anomaly_type, anomalyData.severity, anomalyData.source, anomalyData.entity_type || null, anomalyData.entity_id || null, JSON.stringify(anomalyData.evidence), anomalyData.confidence, anomalyData.detected_at, anomalyData.status || 'new', anomalyData.metadata ? JSON.stringify(anomalyData.metadata) : null);
    ...
    recordAnomalyEvent(anomaly_id, event_type, event_data = {}) {
      ...
      VALUES (?, ?, ?, ?)
    `);
        stmt.run(anomaly_id, event_type, JSON.stringify(event_data), new Date().toISOString());
    }
    /**
     * Get anomaly history
     * @param {string} anomaly_id - Anomaly identifier
    ...
    `);
        stmt.run(proposalData.proposal_id, proposalData.proposal_type, proposalData.objective_id || null, proposalData.anomaly_id || null, JSON.stringify(proposalData.suggested_intent), proposalData.rationale, JSON.stringify(proposalData.risk_assessment), proposalData.confidence, proposalData.created_at, proposalData.status || 'pending', proposalData.expires_at, proposalData.metadata ? JSON.stringify(proposalData.metadata) : null);
        // Record creation event
    ...
    `);
        ...
        stmt.run(newStatus, decision.reviewed_by, decision.reviewed_at, JSON.stringify(decision), proposal_id);
        // Record review event
    ...
    recordProposalEvent(proposal_id, event_type, event_data = {}) {
      ...
      VALUES (?, ?, ?, ?)
    `);
        stmt.run(proposal_id, event_type, JSON.stringify(event_data), new Date().toISOString());
    }
    /**
    ...
    `);
        stmt.run(approval.approval_id, approval.execution_id, approval.plan_id, approval.step_id, approval.intent_id, approval.required_tier, approval.required_by, approval.status, approval.requested_at, approval.requested_by, approval.expires_at, approval.reviewed_by, approval.reviewed_at, approval.decision_reason, approval.action_summary, approval.risk_summary, JSON.stringify(approval.target_entities), approval.estimated_duration_ms, approval.rollback_available ? 1 : 0, approval.created_at, approval.updated_at);
        return approval;
    }
    /**
     * Get approval by ID
