...
const { ARTIFACT_TYPES, ARTIFACT_STATUS, INVESTIGATION_STATUS, validateArtifact, validateInvestigation, normalizeArtifactFilters, normalizeInvestigationFilters, } = require('./workspace-schema');
class WorkspaceManager {
    ...
    `);
        stmt.run(investigation.investigation_id, investigation.name, investigation.description, investigation.objective_id, investigation.incident_id, investigation.status, investigation.created_by, investigation.created_at, investigation.resolved_at, investigation.archived_at, investigation.environment, investigation.metadata_json);
        // Create filesystem directory
        const investigationDir = path.join(this.workspaceRoot, 'investigations', this._sanitizeName(name));
    ...
    `);
        ...
        return {
            ...
            workspace_path: path.join(this.workspaceRoot, 'investigations', this._sanitizeName(row.name)),
        };
    }
    /**
    ...
    `);
        stmt.run(artifact.artifact_id, artifact.artifact_type, artifact.artifact_path, artifact.parent_investigation_id, artifact.intent_id, artifact.execution_id, artifact.objective_id, artifact.incident_id, artifact.content_hash, artifact.size_bytes, artifact.mime_type, artifact.status, artifact.created_by, artifact.created_at, artifact.archived_at, artifact.deleted_at, artifact.environment, artifact.metadata_json);
        return artifact;
    }
    /**
     * Get artifact by ID
    ...
    updateInvestigationStatus(investigation_id, status, updated_by) {
      ...
      WHERE investigation_id = ? AND environment = ?
    `);
        stmt.run(updates.status, updates.resolved_at || null, updates.archived_at || null, updates.updated_at, investigation_id, this.environment);
