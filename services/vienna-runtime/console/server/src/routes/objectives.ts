...
export function createObjectivesRouter(
  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      const status = req.query.status as 'active' | 'blocked' | 'completed' | 'failed' | 'cancelled' | undefined;
  ...
  router.get('/:id/timeline', async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Parse query params
      const category = req.query.category as 'execution' | 'objective' | 'alert' | 'system' | undefined;
      const status = req.query.status as 'info' | 'running' | 'success' | 'warning' | 'error' | undefined;
